	...
	class pageDesigner
	{
		public function createMenu( $menu_class, & $menu_items, $placeholder='' )
		{
			...
			*/
			...
			if (strpos($cur_address, 'admin') !== false) {
			    $adminmobilecss='';
}else{
     $adminmobilecss='scroll-mob-nav';
    
}
		
			$html = '<div id=""><ul class="'.$menu_class.'">';
		...
		public function createMenusearch( $menu_class, & $menu_items, $placeholder='' )
		{
			/*
			* @param $menu_class - feed-navigation( left menu ), tabs-navigation (tab menu)
			* @param $menu_items - Array( array('url'=>'', 'css_class'=>'', 'title'=>'', 'tab_state'=>'') );
			...
			*/
			...
			foreach( $menu_items as $item ){
				$html .= $this->createMenuLinksearch($item);
			}
		...
		public function createMenuLinksearch( $item )
		{
			global $C, $page;
			...
			return '<li class="deletecls-'.$item['id'].'"><a  href="'. $C->SITE_URL . $item['url'] .'" class="'. (isset($item['css_class'])? $item['css_class'] : '' ) . (($item['url'] == $cur_address)? ' selected':'') .'"><span>'. $item['title'] .'</span>'. ($tab_state? '<span class="new-items-count"><span>'. $tab_state .'</span></span>' : '') .'</a><a class="delete" onclick="deletesearch('.$item['id'].')"></a></li>';
		}
		...
		public function loadCSSData()
		{
			...
			if( $C->THEME !== 'default' && $page->is_mobile !==true ){
				if ($handle = @opendir($C->INCPATH.'../themes/'.$C->THEME.'/css/')) {
		
					while (FALSE !== ($entry = readdir($handle))) {
						if( $entry !== '.' && $entry !== '..' ){
							$html .= '<link href="'.  str_replace("/m/", "/",$C->SITE_URL) .'themes/'. $C->THEME .'/css/'. $entry .'?v='. $C->VERSION .'" type="text/css" rel="stylesheet" />'."\n";
						}
					}
		...
		public function loadNetworkLogo()
		{
			...
			if( $C->HDR_SHOW_LOGO==2 && !empty($C->HDR_CUSTOM_LOGO) ) {
				...
				$html .= '<img src="'. $C->STATIC_URL .'images/logo.jpg" alt="'. htmlspecialchars($C->SITE_TITLE) .'" width="40" /><img src="'. $C->STATIC_URL .'images/logo.png" alt="'. htmlspecialchars($C->SITE_TITLE) .'" />';
			}
		...
		public function createInfoBlock( $title, $content, $margintop = false)
		{
			if( !is_string($content) || empty($content) ) return '';
			return '<div class="section-container '.($margintop? 'margin-top15' : '').'">'.((isset($title) && !empty($title)) ? '<h3 class="section-title">'. $title .'</h3>' : ''). $content .'</div>';
		}
		...
		public function createUserLinks($user_array, $size = 'thumbs2')
		{
			...
			foreach( $user_array as $u ){
				$html .= '<a href="'. userlink($u['username']) .'" class="slimuser" title="'. htmlspecialchars($u['username']) .'"><img width="30px" src="'. $C->STORAGE_URL .'avatars/'.$size.'/'. $u['avatar'] .'" alt=""/></a>';
			}
		...
		public function errorMessageforgot($title, $text, $closebtn=TRUE, $incss='')
		{
		
			$html	= '
				<div class="system-message error" style="margin-top:100px;">
					<strong>'.$title.'</strong>'.$text.'
				</div>';
			return $html;
		}
