<?php
	class htmlBlock extends templateElement
	{
		...
		public function __construct( $block_name, $plugin = FALSE, & $layout )
		{
			...
			if($this->page->is_mobile == 'true'){
				$this->path_to_block 			= $plugin? $C->INCPATH.'../apps/'.$plugin.'/mobile/templates/blocks/' : $C->INCPATH.'../themes/'.$C->THEME.'/templates/mobile/blocks/';	
			}else{
		...
		public function save( $layout_part, $remove_vars = FALSE )
		{
			...
			if( $remove_vars ){
				...
				//$this->plugin_manager->invalidateVars(); //tova e slojeno zaradi povtoreniqta //develop 
			}
		...
		public function saveInBlockPart( $block_part, $remove_vars = FALSE )
		{
			...
			if( $remove_vars ){	
				...
				$this->assigned_vars = $tmp; unset($tmp); //added
				$this->plugin_manager->invalidateSpecific($this->assigned_vars); //develop
				$this->removeTemplateVars();
			}
