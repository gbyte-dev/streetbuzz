<?php
	class tableCreator
	{
		...
		public function textFieldSpecial( $row_name, $row_content1,$row_content2,$row_content3, $cssClass='' )
		{
			return '<tr class="'.$cssClass.'">
					<td class="field-title">'. $row_name .'</td>
					<td>'. $row_content1 .'</td>
		...
		public function checkBoxSpecial( $row_name, $checkbox_elements)
		{
	
			foreach( $checkbox_elements as $v ){
				$html = $row_name .'<input type="checkbox" name="'.$v[0].'" value="'.$v[1].'" '.($v[1] == $v[3]? 'checked' : '').' /> <span>'.$v[2].'</span>';
			}
		...
		public function submitButton( $name, $value, $class = 'blue' )
		{
			return '<tr>
					<td></td>
					<td><button type="submit" name="'.$name.'" class="btn '.$class.'"><span>'.$value.'</span></button></td>
				</tr>';
		}
	}