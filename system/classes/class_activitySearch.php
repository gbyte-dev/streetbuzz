<?php
	class activitySearch extends activity
	{
		...
		private function _validadevelopring()
		{
			$string = '';
		...
		public function setPostsQuery()
		{
			...
			$string = $this->_validadevelopring();
			if( empty($string) ){
				return;
			}
			...
			switch( $this->tab )
			{
				...
				case 'tags':
					...
					break;
					case 'location':
						if(is_numeric($this->page->param('s'))){
						$r      = $this->db2->query('SELECT p.location FROM posts as p
											 WHERE p.id="'. $this->page->param('s') . '" ', FALSE);
					  $result = $this->db2->fetch_object($r);
					  $string =  $result->location;
					$this->query = 'SELECT *, "public" AS `type`, p.id AS pid FROM posts as p  WHERE p.location="'.$string.'"  ';
					
						}else{
						$string = 	urldecode($this->page->param('s'));
						$this->query = 'SELECT *, "public" AS `type`, p.id AS pid FROM posts as p  WHERE p.location like "%'.$string.'%"  ';
							
						}
				
							$this->query_order =  ' ORDER BY id DESC ';
		...
		public function loadlocations($result_page = FALSE){
			if(is_numeric($this->page->param('s'))){
				 $r      = $this->db2->query('SELECT p.location FROM posts as p
	                                     WHERE p.id="'. $this->page->param('s') . '" ', FALSE);
        $result = $this->db2->fetch_object($r);
