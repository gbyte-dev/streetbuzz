	...
	class pageDesignerSystem extends pageDesigner
	{
		public function dropDownMenu( $link_title, $menu_items = array(), $dropdown_css_class='', $menu_options_css_class='', $tooltip = false)
		{
			...
			foreach( $menu_items as $item ){
				...
				$item['url'] 	= ( strpos($item['url'], 'https://') === FALSE )? 'https://'.$item['url'] : $item['url'];
		...
		public function createTagLinks($tag_array, $search_where = 'tags')
		{
		   ...
		   if($search_where =='tags'){
			...
			foreach( $tag_array as $t ){
				$html .= '
				<a href="'. $C->SITE_URL .'search/tab:'.$search_where.'/s:'. $t .'" title="#'. htmlspecialchars($t) .'"><li><small>#</small>'. htmlspecialchars(str_cut($t,25)) .'</li></a>';
			}
			$html .= '</ul></div>
		...
		public function usersSettingsMenu( $user_id, $follow, $regime='friendship', $action = '' )
		{
			...
			switch( $regime ){
				...
				case 'friendship' : 
					$follow ?
						$html = '<a class="action-btn user-action add" data-action="follow" data-value="'.$user_id.'" data-namespace="users" data-role="services"><span class="tooltip"><span class="glyphicon glyphicon-user user-icon-flw"></span></span></a>' :
						$html = '<a class="action-btn user-action disconnect-user" data-action="unfollow" data-value="'.$user_id.'" data-namespace="users" data-role="services"><span class="tooltip"><span class="glyphicon glyphicon-user user-icon-flw"></span></span></a>';
					break;
		...
		public function usersSettingsMenu_forpost( $user_id, $follow, $regime='friendship', $action = '' )
		{
			global $user;
			...
			switch( $regime ){
				...
				case 'friendship' : 
					$follow ?
						$html = '<a class="action-btn user-action add" data-action="follow" data-value="'.$user_id.'" data-namespace="users" data-role="services">Unfollow</a>' :
						$html = '<a class="action-btn user-action disconnect-user" data-action="unfollow" data-value="'.$user_id.'" data-namespace="users" data-role="services">Follow</a>';
					break;
