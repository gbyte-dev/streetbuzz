...
class API
{
	...
	public function fcmtoken($userid, $token){
	   
	    
	    	global $db2, $C;
		$db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
      	$query ='SELECT * FROM oauth_access_token WHERE user_id="' . intval($userid) . '"';
      	$res_1 = $db2->query($query);
       
       
	if ($db2->num_rows($res_1) > 0) {
	    
	     $res = $db2->query('Update oauth_access_token SET device_token = "' . $token . '" WHERE  user_id="' . intval($userid) . '"');
	     
	 }else{
	    
	    
	     $res = $db2->query('Insert into oauth_access_token (device_token,user_id) VALUES ("' . $token . '", "' . $userid . '")');
	...
	public function event_post($userid, $title, $start_date, $end_date, $start_time, $end_time, $venue, $geoloc, $describe, $hash_tag, $group, $user, $link, $images, $files, $parentid)
	{
		...
		$res = $db2->query('Insert into posts (user_id,group_id,date,date_lastedit,date_lastcomment,ip_addr,parent_id,post_level,location,posttype,attached) VALUES ("' . $userid . '","' . '0' . '", "' . time() . '", "' . '0' . '","' . time() . '", "' . '0' . '", "' . $parentid . '", "' . $post_level . '","' . $venue . '", 4, "' . '1' . '")');
		...
		if(!empty($geoloc)) {
			...
			$latitude = isset($geoloc_arr[0]) ? $db2->escape($geoloc_arr[0]) : '';
			$longitude = isset($geoloc_arr[1]) ? $db2->escape($geoloc_arr[1]) : '';
		}

		$res = $db2->query('Insert into events (created_at,modified_at,group_id,admin_id,event_type,display_type,address,event_name,location,event_description,start_date,start_time,end_date,end_time,publish_now,publish_date,is_private,status,street_group,street_user,tag_name,url, latitude, longitude) VALUES ("' . date("Y-m-d h:i") . '", "' . date("Y-m-d h:i") . '", "' . '0' . '", "' . $userid . '", "' . 'Normal' . '", "' . 'community' . '", "' . $venue . '", "' . $title . '", "' . $geoloc . '", "' . $describe . '", "' . $start_date . '", "' . $start_time . '", "' . $end_date . '", "' . $end_time . '", "' . '1' . '", "' . '0' . '", "' . '0' . '", "' . '1' . '", "' . $group . '", "' . $user . '", "' . $tags . '", "' . $link . '","'.$latitude.'","'.$longitude.'")');
		...
		$query = 'SELECT * FROM users_followed WHERE whom = ' . $userid .' AND who != '.$userid.' group by who' ;
		$res =  $db2->query($query);
    ...
    <!-- end : event hashtag -->';
			...
			foreach ($images as $img) {
				...
				if ($ii = $p->attach_image($C->STORAGE_TMP_DIR . $img["image"], $img["image"])) {
					...
					foreach ($newimg as $tmpimg) {
						if(file_exists($C->STORAGE_TMP_DIR.$tmpimg) ) {
	 ...
	 // notification start
            $followers=$db2->query('select who FROM users_followed WHERE whom='.$userid.'');
	...
	$standardnotifytype='ntf_me_if_u_follow_buzz_event';
       ...
       /*$newisert =$network->insert_active_profilenotifications1($vl[0],$post_id,$notifytype,$type,$standardnotifytype);
        print_r($vlt); die("=======");*/
          ...
          $sql_user1 = 'SELECT * FROM users WHERE  id="' .$userid. '"';
      $res_user1 = $db2->query($sql_user1);
	 $obj_user1 = $db2->fetch_object($res_user1);
    ...
    //$db2->query(
 	...
 	$notif_object_id =$userid;
		...
		$db2->query('insert into notifications  (notif_type, to_user_id, in_group_id, from_user_id,notif_object_type,notif_object_id,date) values  ("'.$standrdtype.'","'. $ownuserid .'","'.$groupid.'","'.$userid.'","'.$notif_object_type.'","'.$postid.'","'.$date.'")');
	...
	public function event_edit($user_id, $post_id, $title, $start_date, $end_date, $start_time, $end_time, $describe, $hash_tag, $group, $user, $link, $images, $venue, $geoloc)
	{
		...
		$db2->query('Update events SET modified_at = "' . date("Y-m-d h:i") . '",event_name = "' . $title . '",event_description = "' . $describe . '" ,start_date = "' . $start_date . '",start_time = "' . $start_time . '",end_date = "' . $end_date . '",end_time = "' . $end_time . '",url = "' . $link . '",url = "' . $link . '",tag_name = "' . $tags . '",street_group = "' . $group . '",street_user = "' . $user . '", latitude="'.$latitude.'", longitude="'.$longitude.'", location="'.$geoloc.'", address="'.$venue.'" WHERE  id="' . $event_id . '"');
	...
	*/
	public function mypolls_workspace($userId, $pagenumber, $pagerecordcount)
	{
		...
		$db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
		$pollmyquery  = $db2->query('SELECT  
						p.id as postid, p.user_id AS postuserid,
                        u.username AS postusername,
                        u.avatar AS postuserimage,
						if( (u.cover is null), "", u.cover) AS coverimage,
					...
					FROM `posts` as p 
					inner join polls as ps ON p.id = ps.posts_id 
	...
	*/
	...
	public function ntf_me_if_u_share_buzz($userid, $respo)
	{
   	...
   	public function ntf_me_if_u_follow_buzz($userid, $respo)
	{
		// echo "aa"; die;
		global $db2, $C;
		$db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);

		$res = $db2->query('Update users_notif_rules SET ntf_me_if_u_follow_buzz = "' . $respo . '" WHERE  user_id="' . $userid . '"');
