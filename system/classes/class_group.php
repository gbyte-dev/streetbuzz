	...
	class group
	{
		...
		public function lastUsers()
		{
			...
			while( $o = $this->db2->fetch_object() ){
				$ladevelop = array( 'username' => $o->username, 'date' => $o->date, 'avatar'=>(!empty($o->avatar)? $o->avatar : $GLOBALS['C']->DEF_AVATAR_USER) );
			}
		}
		...
		public function createGroup()
		{
			...
			if( ! $error ) {
				$categories = $this->getGroupCategories();
				if(!empty($group_category) && !isset($categories[$group_category])){
					$error	= TRUE;
					$errmsg	= 'group_setterr_no_category';
				}
				
			}
			
			if( ! $error ) {
				$this->db2->query('INSERT INTO groups SET groupname="'.$this->db2->e($group_alias).'", title="'.$this->db2->e($group_name).'", about_me="'.$this->db2->e($group_description).'", is_public="'.($group_type=='public'?1:0).'", category_id="'.(int) $group_category.'"');
		...
		public function getGroupCategories($without_locked = false, $get_first = true)
		{
			global $C;
			
			if($get_first){
				$categories = array(0 => (object) array('id' =>0, 'title' => $this->page->lang('global_none'), 'url'=>0, 'is_admin_created'=>1, 'is_locked'=>0 ));
			}else{
				$categories = array();
			}
			
			$check_locked = $without_locked && $this->user->is_logged && $this->user->info->is_network_admin == 0? ' WHERE is_locked=0 ' : '';
				
			$r = $this->db2->query('SELECT * FROM groups_categories '.$check_locked.' ORDER BY is_admin_created DESC');
			while( $o = $this->db2->fetch_object($r) ) {
				$categories[$o->id] = $o;
			}
		...
		public function createGroupCategory($category_name)
		{
			...
			$is_admin = $this->user->info->is_network_admin == 1? 1 : 0;
			$creator_id = (int) $this->user->id;
			$url = str_replace(' ', '', strtolower($category_name));
			
			$r = $this->db2->query('INSERT INTO groups_categories(title, url, is_admin_created, creator_id, is_locked) 
					VALUES("'.$this->db2->e($category_name).'", "'.$this->db2->e($url).'", '.$is_admin.', '.$creator_id.', 0)');
		...
		public function getCategoryByUrl($url)
		{
			...
			$r = $this->db2->query('SELECT * FROM groups_categories WHERE url="'.$this->db2->e($url).'" LIMIT 1');
			$o = $this->db2->fetch_object($r);
			
			return $o;
		}
		
		public function changeCategory($category_id, $categories)
		{
			global $C;
			
			if(!isset($categories[$category_id])){
				return 'newcat_invalid';
			}
			
			$this->db2->query('UPDATE groups SET category_id = '. (int) $category_id.' WHERE id='.(int) $this->group->id);
