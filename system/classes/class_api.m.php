<?php
 	class API
	{
		...
		
		{
			...
			$upload_dir = $C->SITE_URL.'storage/attachments/1/';
            ...
            if ((int)$parentid==0)
            {
 			...
 			$db2->query('Insert into posts (user_id,message,date,date_lastcomment,ip_addr,parent_id,post_level,attached) VALUES ('.$user_id.',"'.mysqli_real_escape_string($link,$postdata).'","'.time().'","'.time().'","'.ip2long($_SERVER['REMOTE_ADDR']).'",'.$parentid.','.$post_level.','.count($images).')');
...
//
			...
			$p = new newpost();
 			if( isset($images) ){ 
					foreach($images as $img){
					    			...
					    			//print_r($upload_dir.$img["image"]);
					    			
					    			

  						if( $ii = $p->attach_image($upload_dir.$img["image"], $upload_dir."post".$img["image"]) ) {
				...
				//	move_uploaded_file( $img["image"],$upload_dir.$img["image"]);
					unset($images);
				}
			//exit;	
			/*foreach ($images as $image) {
				...
				//$imagestring = 'O:8:"stdClass":8:{s:5:"title";s:62:"Copy.jpeg";s:13:"file_original";s:25:"1589049386381290_orig.jpg";s:12:"file_preview";s:26:"1589049386381290_large.jpg";s:14:"file_thumbnail";s:26:"1589049386381290_thumb.jpg";s:13:"size_original";a:2:{i:0;i:562;i:1;i:1218;}s:12:"size_preview";a:2:{i:0;i:231;i:1;i:500;}s:8:"filesize";i:47448;s:4:"hits";i:0;}';
				
				$imagestring = 'O:8:"stdClass":8:{s:5:"title";s:62:"'.$imagename.'";s:13:"file_original";s:25:"'.$imagename.'";s:12:"file_preview";s:26:"'.$imagename.'";s:14:"file_thumbnail";s:26:"'.$imagename.'";s:13:"size_original";a:2:{i:0;i:562;i:1;i:1218;}s:12:"size_preview";a:2:{i:0;i:231;i:1;i:500;}s:8:"filesize";i:47448;s:4:"hits";i:0;}';

				//echo  'INSERT INTO `posts_attachments`(`post_id`, `type`, `data`,  `content`) VALUES ('.$post_id.',\'Image\',\''.$imagestring.'\',\''.$imagecaption.'\')';

			    $db2->query('INSERT INTO `posts_attachments`(`post_id`, `type`, `data`,  `content`) VALUES ('.$post_id.',\'Image\',\''.$imagestring.'\',\''.$imagecaption.'\')');
		...
		public function updateCoverImage($userid,$cover)
		{
			global $db2, $C;
	        $db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
	 ...
	 //$avatar_name,str_replace(' ', '-',strtolower($random_name)), str_replace(' ', '-',strtolower($server_url.$random_name))
	   ...
	   $db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
		...
		private function getAllFollowersUserID($userid)
		{
 		...
 		public function submitPost($user_id,$postdata,$parentid)
		{
            ...
            if ((int)$parentid==0)
            {
 			...
 			$db2->query('Insert into posts (user_id,message,date,date_lastcomment,ip_addr,parent_id,post_level) VALUES ('.$user_id.',"'.mysqli_real_escape_string($link,$postdata).'","'.time().'","'.time().'","'.ip2long($_SERVER['REMOTE_ADDR']).'",'.$parentid.','.$post_level.')');
            ...
            {
				$series = 'a:2:{i:0;s:5:"'.$parentid.'";i:1;i:'.$post_id.';}';
								echo 'INSERT INTO `post_replay`(`parent_id`, `alternate_parent_id`, `replay_id`, `action_type`, `series`) VALUES ('.$parentid.','.$parentid.','.$post_id.',"buzz","'.$series.'")';
	       ...
	       $res =  $db2->query($sql);
			...
			foreach ($array as $i =>$array_expression)
            {
               ...
               // $array[$i]["like"] = $this->getUserLikes($userid);
                ...
                //$array[$i]["cover_image"] = $C->SITE_URL.'storage/avatars/'.$array[$i]["cover_image"];
	  ...
	  {	      global $db2;
	    ...
	    function editUser($userid,$fullname,$dob,$gender,$about_me,$location)
	    {
	   	    global $db2, $C;
 	        $db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
 	            $tmplang	= $db2->fetch_field('SELECT value FROM settings WHERE word="LANGUAGE" LIMIT 1');
	...
	function editUser1($userid,$fullname,$email,$phone,$username,$password,$dob,$gender,$location,$about_me)
	    {
	   	    global $db2, $C;
 	        $db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
 	            $tmplang	= $db2->fetch_field('SELECT value FROM settings WHERE word="LANGUAGE" LIMIT 1');
	...
	function editUserPhone($userid,$phone)
	    ...
	    public function generateToken($userid, $device_token=NULL)
	    {
 		    global $db2, $C;
	        $db2 = new mysql($C->DB_HOST, $C->DB_USER, $C->DB_PASS, $C->DB_NAME);
		    ...
		    if($db2->num_rows($res) > 0)
		    {
			    ...
			    $res = $db2->query('Update oauth_access_token SET access_token = "'.$oauth_access_token.'", device_token="'.$device_token.'" WHERE  id="'.intval($obj->id).'"');
			    
			    return $oauth_access_token;
		    }
		    else
		    {
		         $res = $db2->query('Insert into oauth_access_token (access_token,user_id,device_token) VALUES ("'.$oauth_access_token.'", "'.$userid.'", "'.$device_token.'")');
                return $oauth_access_token;
    		}
	    }
	  ...
	  public function loadPosts($user_id,$pagenumber,$pagerecordcount)
	  {
			...
			$query = 'SELECT 
						...
						if( (pv.cnt is null), 0, pv.cnt) AS ViewCount
							FROM post_userbox b
							LEFT JOIN posts p ON pz.id=b.post_id
