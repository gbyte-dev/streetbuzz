	  ...
	  public function loadPosts($user_id,$pagenumber,$pagerecordcount)
	  {
			...
			$query = 'SELECT 
                        ...
                        if( ((Select count(id) from  posts_social_share where post_id =p.id and user_id=b.user_id) > 0), true,  false) AS isshared
							...
							LEFT OUTER JOIN post_views_list pv ON pv.post_id = p.id
						WHERE
                        b.user_id='.$user_id.' AND p.post_level = 0  AND p.id is not null
                            Group By p.id
                            ORDER BY p.date_lastcomment DESC
                         LIMIT '.$pagenumber.','.$pagerecordcount;
            $res =  $db2->query($query);
			...
			foreach ($array as $i =>$array_expression)
            {
		      ...
		      if(empty($obj3->id)){
				...
				/*Ads Logic */
                 $checkads =$this->checkcommercialadds($user_id,$array[$i]['date']);
                 if( !empty($checkads) ){
                    $array[$i]["showcommercialadd"] = true;  
                    $array[$i]["commercialadddata"] = $checkads;  
                }else{
                    $array[$i]["showcommercialadd"] = false; 
                    
                }
				$checkparagaphexist1 =$this->checkparagaphexist1($user_id);
			    if( !empty($checkparagaphexist1) ){
					$array[$i]["showparagraph1"] = true; 
					$array[$i]["showparagraph1data"] = $checkparagaphexist1; 
				}else{
					$array[$i]["showparagraph1"] = false; 
					
				}
				$checkparagaphexist2 =$this->checkparagaphexist2($user_id);
				if( !empty($checkparagaphexist2) ){
					$array[$i]["showparagraph2"] = true; 
					$array[$i]["showparagraph2data"] = $checkparagaphexist2; 
				}else{
					$array[$i]["showparagraph2"] = false; 
				}
                $checkparagaphexist3 =$this->checkparagaphexist3($user_id);
                if( !empty($checkparagaphexist3) ){
                    $array[$i]["showparagraph3"] = true; 
                    $array[$i]["showparagraph3data"] = $checkparagaphexist3; 
                }else{
                	$array[$i]["showparagraph3"] = false; 
                	    
                }
                $checkadexist =$this->checkadexist($user_id);
                if(!empty($checkadexist)){
                	 $array[$i]["officialadd"] = true;
                	 $array[$i]["officialadddata"] = $checkadexist; 
                }else{
                	 $array[$i]["officialadd"] = false; 
	  ...
	  public function loadVideoNews($user_id,$pagenumber,$pagerecordcount)
	  {
			...
			$query = 'SELECT 
                        ...
                        if( ((Select count(id) from  posts_social_share where post_id =p.id and user_id=b.user_id) > 0), true,  false) AS isshared
							...
							LEFT OUTER JOIN post_views_list pv ON pv.post_id = p.id
						WHERE
                        b.user_id='.$user_id.' AND p.post_level = 0 AND p.posttype = 3 AND p.id is not null
                            Group By p.id
                            ORDER BY p.date_lastcomment DESC
                         LIMIT '.$pagenumber.','.$pagerecordcount;
            $res =  $db2->query($query);
 ...
 LEFT JOIN users u ON u.id=p.user_id
	 LEFT OUTER JOIN posts_attachments pa ON pa.post_id = p.id
   	...
   	where e.admin_id="'.$userId.'" AND 
   	  ep.edit_status IS NULL   and (p.post_level is null or p.post_level ="0")
   	 
   	  order by p.date_lastcomment desc,ep.id desc LIMIT '.$pagenumber.','.$pagerecordcount;
	   ...
	   public function userLoadPosts($user_id,$pagenumber,$pagerecordcount)
	  {
			...
			$query = 'SELECT 
                        ...
                        if( ((Select count(id) from  posts_social_share where post_id =p.id and user_id=p.user_id) > 0), true,  false) AS isshared
							...
							LEFT OUTER JOIN post_views_list pv ON pv.post_id = p.id
						WHERE
                        p.user_id='.$user_id.' AND p.post_level = 0  AND p.id is not null
                            Group By p.id
                            ORDER BY p.date_lastcomment DESC
                         LIMIT '.$pagenumber.','.$pagerecordcount;
