			...
			switch( $this->tab )
			{
				...
				case 'posts':	//default: 
								$this->query = 'SELECT p.*, p.id AS pid, "public" AS `type` 
									FROM posts p 
									LEFT JOIN posts_mentioned pm ON pm.post_id = p.id AND pm.user_id = "'.$this->target_user->id.'"
									WHERE (p.user_id="'.$this->target_user->id.'" ) '. $this->filter->groups('group_id') .' AND p.api_id<>2 AND p.api_id<>6 ';
