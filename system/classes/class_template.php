	...
	class template extends templateElement
	{
		...
		public function __construct( $params = array(), $default_load = TRUE )
		{
			...
			if( $this->default_load ){
				...
				$this->pm->onPageLoad();
				if(strpos($_SERVER["REQUEST_URI"], 'signin/forgotten') > 0 ){
				    $this->useLayout( (!$this->page->is_mobile && !$this->user->is_logged && !$this->user_or_group_check)? 'header' : 'header' );
				...
				$this->initRoutine(!$this->page->is_mobile? (($this->user->is_logged || $this->user_or_group_check)? 'DefaultHeader' : 'DefaultHeader') : 'MobileHeader');
				foreach( $params as $k=>$v ){
					$this->routine->setSpecific( '{%'. $k . '%}', $v );
				}
				$this->routine->load();		
				if(strpos($_SERVER["REQUEST_URI"], 'signin/forgotten') > 0 ){
				$this->useLayout('newpage_layout');
				}else{
