 ...
 $retarr = get_access_token_yahoo(OAUTH_CONSUMER_KEY, OAUTH_CONSUMER_SECRET, $request_token, $request_token_secret, $oauth_verifier, false, true, true);
  ...
  $guid    =  $body_parsed['xoauth_yahoo_guid'];
   ...
   $access_token  = rfc3986_decode($body_parsed['oauth_token']) ;
	...
	foreach($retarrs as $keys=>$vals){
	

	if(is_valid_email($vals['email'])) {
	$res[] = $vals['email'];
	}	
...
$res = implode(",",$users);
 ...
 <input type="hidden" name="users" value="<?php echo $res;?>" id="users">
  <input type="hidden" name="userid" value="<?php echo $_SESSION['id'];?>" id="users">
 <input type="hidden" name="password" value="<?php echo $street_userpassword;?>" id="users">
