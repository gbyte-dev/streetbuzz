...
$db2->query("UPDATE sb_reporterpayment
SET user_id=$userId, transaction_type='$transtion_type',payemnt_month_year='$month_year',payment_amount=$transtion_amount,refrence_id='$refrence_number',payment_date='$date',entry_date=now()
WHERE id=$id;");
...
if(isset($_GET['id'])){
    $id=$_GET['id'];
$qq=$db2->query("SELECT * FROM `sb_reporterpayment` where id=$id");
$row = $qq->fetch_assoc();

     $user_id=$row["user_id"];
  
    $u=$db2->query("SELECT * FROM `users` where id=$user_id");
    $r = $db2->fetch_object($u);
    $username=$r->username;
}
$newscontent='
<div class="row">
...
<input type="text" id="user" class="form-control" name="uname" placeholder="Enter Username" value="'.$username.'" required>
<input type="hidden" id="user_id"  name="uname_id" value="'.$user_id.'">
...
<input type="text" class="form-control" name="transtion_amount" placeholder="Enter Transaction Amount" value="'.$row['payment_amount'].'" required>
</div>

<div class="col-md-4">
<b>Refrence Number<span style="color:red">*</span></b>
<input type="text" class="form-control" name="refrence_number" placeholder="Refrence Number" value="'.$row['refrence_id'].'" required>
</div>
<div class="col-md-4">
<b>Payment Date<span style="color:red">*</span></b>
<input type="datetime-local" class="form-control" name="date" value="'.$row['payment_date'].'" required>
</div>

<div class="col-md-4">
<b>Payment Month and Year<span style="color:red">*</span></b>
  <input type="month" class="form-control"  name="month_year" value="'.$row['payemnt_month_year'].'" required>
</div>

<div class="col-md-4 " style="margin-top: 15px;">
<button type="submit" class="btn btn-info btn-lg pull-right" name="updatePeyment">Submit</button>
</div>
</form>
