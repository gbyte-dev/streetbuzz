<?php
  ...
  if(isset($_POST['savesubmit'])){
      ...
      $db2->query('INSERT INTO country(name) VALUES( "'.$country.'")');
  ...
  if(isset($_POST['updatesubmit'])){
    $id=$_POST['id'];
    $country=$_POST['country'];
    $db2->query('UPDATE country SET name="'.$country.'" WHERE id= "'.$id.'"');
  }

    $datares='
    <form method="POST" >
    <div class="row">
  <div class="col-xs-6 col-md-6"><input class="form-control" type="text" name="country" placeholder="country" required/></div>
  <div class="col-xs-6 col-md-3"><button type="submit"  name="savesubmit" class="btn btn-primary mb-3">Submit</button></div>
</div>  
 <div class="row">
...
<link rel="stylesheet" href="https://cdn.datatables.net/1.10.9/css/jquery.dataTables.min.css">
  ...
  if (isset($_GET['action'])){
        $id=$_GET['id'];
       $db2->query('DELETE FROM `country` where id="'.$id.'"');
  }

  $qq=$db2->query('SELECT * FROM `country`');
  //$querry=mysqli_query($con,$qq);
  $x=1;
  $aa="'Are You Sure'";
  $fordelete='onclick="return confirm('.$aa.')"';
    foreach ($qq as $value) {
        ...
        <td>'.$x.'</td>
        <td>'.$value['name'].'</td>
        <td><a class="btn btn-danger" '.$fordelete.' href="country?id='.$value['id'].'&action=delete"> Delete</a>
          <button type="button" data-id="'.$value['id'].'" data-country="'.$value['name'].'" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal">Edit</button></td>
        </tr>';
        $x++; }
  $datares.='</tbody>
</table>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
           <form method="POST" >
    <div class="row">
  <div class="col-xs-6 col-md-6"><input class="form-control country-name" type="text" name="country" placeholder="country" required/>

<input class="form-control country-id" type="hidden" name="id" placeholder="country" required/>
  </div>
  <div class="col-xs-6 col-md-3"></div>
</div>
        </div>
...
';  
  ...
  $newscontent ='<div class="container-fluid"><b>Add Country </b></div>
'.$datares.'
