     ...
     if(count($locationresponse) > 0){
       ...
       if(!empty($_POST['submit'])){
           $status = $_POST['status'];
        	...
        	$wherecondition = 'ads_type = 8';


       }
         
      
	     $check        = $db2->query('select ad.* from ads_info as ad  WHERE '.$wherecondition.' order by ad.id desc ');
 if (isset($_GET['action'])){
        $id=$_GET['id'];
       $db2->query('DELETE FROM `ads_info` where id="'.$id.'"');
       
       $this->redirect('admin/homeaddsdisplay');
  }
	  //<div class="row" style="border-top:1px solid black"><div class="col-md-1">Sl. No</div><div class="col-md-1">Image</div><div class="col-md-2">Customer name</div><div class="col-md-2">District  customer</div><div class="col-md-2">Ad Position</div><div class="col-md-2">Created Date</div><div class="col-md-2">Action</div></div>
  ...
  <tbody>
   ...
   $key =1;
	...
	$adstypedata = '';
	if($ads_type == "1"){
	    $adstypedata = "Classified";
	    
	}
	if($ads_type == "2"){
	    $adstypedata = "Banner Advt";
...
if($views->num_rows > 0){
	...
	$ad_display_type         =$result->ad_display_type;
	if($ad_display_type == "video"){
	    $imagesres ='<video width="70px" src="'.$C->STORAGE_URL.'advs/'.$small_image.'" alt="No image" />';
	}else{
	     $imagesres ='<img width="70px" src="'.$C->STORAGE_URL.'advs/'.$small_image.'" alt="No image">';
  ...
  <div class="col-md-1"><p>'.$key++.'</p></div><div class="col-md-2"><p>'.$imagesres.'</p></div><div class="col-md-1"><p>'.$customer_name.'</p></div><div class="col-md-2"><p>'.$locationname.'</p></div><div class="col-md-2"><p>'.$adstypedata.'</p></div><div class="col-md-2"><p>'.$created_date.'</p></div><div class="col-md-2"><span><a href="'.$C->SITE_URL.'admin/homeaddsedit?id='.$adid.'" class="edit" style="background-color:orange;color:white;border:1px solid orange;padding:4px;">Edit</a></span></div></div>';*/
  $datares    	.='<tr>
        <td>'.$key++.'</td>
        <td>'.$imagesres.'</td>
        <td>'.$customer_name.'</td>
        ...
        <td>'.$adstypedata.'</td>
        <td>'.$created_date.'</td>
        <td><a href="'.$C->SITE_URL.'admin/homeaddsedit?id='.$adid.'" class="edit" style="background-color:orange;color:white;border:1px solid orange;padding:4px;">Edit</a><br><a style="background-color:red;color:white;border:1px solid orange;padding:4px;"  onclick="delete_a('.$adid.')"> Delete</a></td>
        </tr>';
	...
	$newscontent ='<div class="container-fluid"><span style="float:right;"><a href="'.$C->SITE_URL.'/admin/homeaddsinsert?q=home" style="border:1px solid orange;padding: 5px;
    border-radius: 14px;
    background-color: orange;
    color: white;">Create Advt.</a></span></div>
...
';
// '.$datares.'
// <script type="text/javascript" src="'.$C->SITE_URL.'/static/js/jquery.js?v=3.6.0"></script><script type="text/javascript" src="https://streetbuzz.co/test/static/js/jquery-ui.js?v=3.6.0"></script>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
   window.location.href = 'admin/homeaddsdisplay?id='+ id +'&action=delete';
  }
})
	}
