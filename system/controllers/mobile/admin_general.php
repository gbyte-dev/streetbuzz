<?php
	...
	$phone_confirm	= $s->USERS_PHONE_CONFIRMATION;
	...
	$stealth_mode 	= isset($s->STEALTH_MODE)? $s->STEALTH_MODE : 2;
	...
	if( isset($_POST['sbm']) ) {
		...
		if( !$error ){
			...
			$phone_confirm		= isset($_POST['network_phone_confirm']) && $_POST['network_phone_confirm']==1;
			...
			$stealth_mode		= isset($_POST['stealth_mode'])? (($_POST['stealth_mode'] == 1 || $_POST['stealth_mode'] == 2)? (int) $_POST['stealth_mode'] : 1) : 1;
			...
			$db2->query('REPLACE INTO settings SET word="USERS_PHONE_CONFIRMATION", value="'.intval($phone_confirm).'" ');
			...
			$db2->query('REPLACE INTO settings SET word="ADMIN_STEALTH_MODE", value="'.$db2->e($stealth_mode).'" ');
			
			if( isset($_POST['network_name']) ) {
				$network_name	= htmlspecialchars(trim($_POST['network_name']));
			}
			...
			if( isset($_POST['network_email']) ) {
				$system_email	= htmlspecialchars(trim($_POST['network_email']));
			}
	...
	$rows = array(
			...
			$table->radioButton( 'Phone confirmation', 'network_phone_confirm', array('1'=>$this->lang('admgnrl_frm_mobi_on'), '0'=>$this->lang('admgnrl_frm_mobi_off')), $phone_confirm ),
			...
			$table->radioButton( $this->lang('admin_stealth_mode_ttl'), 'stealth_mode', array('1'=>$this->lang('admgnrl_frm_mobi_on'), '2'=>$this->lang('admgnrl_frm_mobi_off')), $stealth_mode ),
			$table->textField( '', $this->lang('admin_stealth_mode_txt') ), $table->textField( '', '' ),
