...
inner join  posts as p ON ep.post_id = p.id		 
	...
	$eventnotifyaccept           = $db2->query('SELECT e.id as eventid,e.event_name,e.address,e.location,e.event_description,e.start_date,e.start_time,e.end_date,e.end_time,e.url,e.status,e.event_type,e.tag_name,pu.event_status,ep.edit_status 
		FROM `post_userbox` as pu 
                    ...
                    inner join posts as p ON p.id=pu.post_id					
					inner join events as e on ep.event_id = e.id 
					WHERE pu.user_id = "'.$this->user->id.'" and (pu.event_status = 1 and pu.status is null) and p.post_level is null order by pu.id desc,ep.created desc LIMIT 10' );
...
$myevent            =$db2->query('SELECT u.id,u.username,u.fullname,u.avatar,ep.post_id,p.date as postdate,p.user_id  FROM event_posts as ep
 ...
 if($myeventres->user_id != $this->user->id){
		...
		$alleventhtml .='
<div class="activity no-comments" id="allcal'.$myeventres->post_id.'" >

<!-- start Parent -->
<div class="row start'.$myeventres->post_id.'" style="border:0px solid red; margin:0px; padding:0px;">

<div id="allcalanderjaneesh2'.$myeventres->post_id.'"></div> 
...
<!--/ start : activity container --><div class="activity-container">
		...
		<div class="activity-header">
			...
			<a href="" data-value="'.htmlentities('{"activities_type":"public","activities_id":"'.$myeventres->post_id.'"}').'" data-role="services" data-namespace="activities" data-action="deleteActivity" ><img src="'.$C->SITE_URL.'static/images/icons/DELETE.png"></a>	
...
style="padding:10px 10px 0px 10px;">
	...
	<div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12 buzz-content event-list-blue-bg">
	...
	for($m=0;$m<count($chield);$m++){
       ...
       style="border:0px solid grey;"><a style="cursor:pointer;" class="pull-right" onclick="replaycontenttimeline('.$myeventres->post_id.','.$chield[$m]->id.','.$eventrew.')">View Replies</a></div>';
 		...
 		if(!empty($is_favres)){
				...
				if(($j-1)==$m){
					$css="tree1";
					$childclass="child".$myeventres->post_id;
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 buzz-parent-box">
                     ...
                     <div class="col-lg-11 col-md-11 col-sm-11 col-xs-11" style="padding:0px 3px 0px 8px;">
                        <div class="activity-container">
                           <div class="activity-header col-lg-12 col-md-12 col-sm-12 col-xs-12"><a href="'.userlink($chield[$m]->username).'" class="author bizcard" data-userid="'.$chield[$m]->userid.'">'. $chield[$m]->username .'</a>
...
<script>
var main =$("#allcal'.$myeventres->post_id.'").height();
var child =$(".child'.$myeventres->post_id.'").height();
var final = Number(main)-Number(child);
$("#allcalanderjaneesh2'.$myeventres->post_id.'").css("height",final);
</script>
...
#allcalanderjaneesh2'.$myeventres->post_id.'{
z-index: 100;
position: absolute; 
...
/******************* End : NOTIFICATION All Events - List View *********************/




/******************* Start : NOTIFICATION My Events - List View *********************/
...
$myevent            =$db2->query('SELECT u.id,u.username,u.fullname,u.avatar,ep.post_id,p.date as postdate  FROM event_posts as ep
 ...
 $url = 'https://www.google.com/maps/search/'.urlencode(htmlentities($vals->address, ENT_COMPAT, 'UTF-8'));
		...
		$myeventhtml .='
		 <div class="activity no-comments" id="mycal'.$myeventres->post_id.'">

<!-- start Parent -->
<div class="row start'.$myeventres->post_id.'" style="border:0px solid red; margin:0px; padding:0px;">

<div id="myeventcalljaneesh2'.$myeventres->post_id.'"></div> 
	...
	for($m=0;$m<count($chield);$m++){
       ...
       style="border:0px solid grey;"><a style="cursor:pointer;" class="pull-right" onclick="replaycontenttimeline('.$myeventres->post_id.','.$chield[$m]->id.','.$eventrew.')">View Replies</a></div>';
 		...
 		if(!empty($is_favres)){
				...
				if(($j-1)==$m){
					$css="tree1";
					$childclass="mycalchild".$myeventres->post_id;
...
<script>
var main = $("#mycal'.$myeventres->post_id.'").height();
var child = $(".mycalchild'.$myeventres->post_id.'").height();
var final =Number(main)-Number(child);
$("#myeventcalljaneesh2'.$myeventres->post_id.'").css("height",final);
...
#myeventcalljaneesh2'.$myeventres->post_id.'{
z-index: 100;
position: absolute; 
border-left: 4px solid orange; 
float:left; 
...
/******************* End : My Events - List View  **********************/


/******************* Start : Accepted Events - List View  **********************/
...
$myevent            =$db2->query('SELECT u.id,u.username,u.fullname,u.avatar,ep.post_id,p.date as postdate  FROM event_posts as ep
 ...
 if($myeventres->avatar !=''){
	 ...
	 $src = $C->SITE_URL.'storage/avatars/thumbs1/_noavatar_user.gif';
		...
		$acceptedhtml .='
		 <div class="activity no-comments" id="acceptcall'.$myeventres->post_id.'">

<!-- start Parent -->
<div class="row start'.$myeventres->post_id.'" style="border:0px solid red; margin:0px; padding:0px;">

<div id="acceptjaneesh2'.$myeventres->post_id.'"></div> 
	...
	for($m=0;$m<count($chield);$m++){
       ...
       style="border:0px solid grey;"><a style="cursor:pointer;" class="pull-right" onclick="replaycontenttimeline('.$myeventres->post_id.','.$chield[$m]->id.','.$eventrew.')">View Replies</a></div>';
 		...
 		if(!empty($is_favres)){
				...
				if(($j-1)==$m){
					$css="tree1";
					$childclass="acceptchild".$myeventres->post_id;
...
<script>
var main = $("#acceptcall'.$myeventres->post_id.'").height();
var child = $(".acceptchild'.$myeventres->post_id.'").height();
var final = Number(main)-Number(child);
$("#acceptjaneesh2'.$myeventres->post_id.'").css("height",final);
...
#acceptjaneesh2'.$myeventres->post_id.'{
z-index: 100;
position: absolute; 
border-left: 4px solid orange; 
float:left; 
