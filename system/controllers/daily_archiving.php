...
$ninety_days_ago = date('Y-m-d', strtotime('-60 days', strtotime(date("Y-m-d")))); 

//$ninety_days_ago1 = date('Y-m-d', strtotime('-60 days', strtotime(date("Y-m-d")))); 
//$startdate = date('Y-m-d H:i:s', strtotime($ninety_days_ago.'00:00:00'));
//$enddate = date('Y-m-d H:i:s', strtotime($ninety_days_ago."23:59:59"));

$startdate = strtotime($ninety_days_ago.'00:00:00');
$enddate = strtotime($ninety_days_ago."23:59:59");


global $db2;

//echo 'SELECT * FROM `posts` WHERE date BETWEEN "'.$startdate.'" AND "'.$enddate.'" order by id ASC LIMIT 2';die;
$query = $db2->query('SELECT * FROM `posts` WHERE date BETWEEN "'.$startdate.'" AND "'.$enddate.'" order by id ASC LIMIT 20');

if ($query->num_rows > 0)
{
    /*Archival Database*/
    $servername = "182.18.139.51";
    $username = "sreeniva_archive";
    $password = "sreeniva_archive";
    $dbname = "sreeniva_archive";
    $moveurl ="";
...
//echo '<br> posts_comments_watch done';
        /*posts mentioned */
...
//echo '<br> posts_spamprotector done';
        /*posts agree  */
