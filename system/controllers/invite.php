<?php
	...
	if( $submit ) {
		...
		if( !$error ) {
			...
			foreach($do_send_invites as $tmp) {
				...
				$from = $this->user->info->email;
				
				$SmtpServer="webmail.streetbuzz.co.in";
                $SmtpPort="25";
                $SmtpUser="srinivasa@streetbuzz.co.in";
                $SmtpPass="SRI@12345678";
					$message =$_POST['name'][0].",\n".
				           $this->user->info->fullname."invites you to join StreetBuzz \n To sign up, use the following link \n".$C->SITE_URL."\n Regards, \n StreetBuzz";
				$SMTPMail = new SMTPClient ($SmtpServer, $SmtpPort, $SmtpUser, $SmtpPass, $from, $tmp->email, $subject, $message);
	...
	//TEMPLATE CODE START
	$tpl = new template( array('page_title' => $this->lang('os_invite_ttl_colleagues', array('#SITE_TITLE#'=>$C->SITE_TITLE, '#OUTSIDE_SITE_TITLE#'=>$C->OUTSIDE_SITE_TITLE)), 'header_page_layout'=>'sc') );
