<?php
	...
	{ 
		...
		$html .= '<meta name="description" content="A blog about Customer Engagement & Business Collaboration software: use cases, best practices and tips.  Engagement/Inbound Marketing and Content Marketing.">'."\n";
	...
	function getCSSData()
	{
		...
		if( !isset($C->THEME_CSS_OVERWRITE) ){
			foreach( $css_files as $css ){
				$html .= '<link href="'. str_replace("/m/","/",$css_location . $css .'.css?v='. $C->VERSION .'" type="text/css" rel="stylesheet" />'."\n");
			}
		}
		...
		if( $C->THEME != 'default' ){
			if ($handle = @opendir($C->INCPATH.'../themes/'.$C->THEME.'/css/')) {
				...
				while (FALSE !== ($entry = readdir($handle))) {
					if( $entry !== '.' && $entry !== '..' ){
						$html .= '<link href="'. str_replace("/m/","/", $C->SITE_URL) .'themes/'. $C->THEME .'/'. $entry .'.css?v='. $C->VERSION .'" type="text/css" rel="stylesheet" />'."\n";
					}
				}
