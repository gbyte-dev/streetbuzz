	...
	//this for subreplay checking 
	 ...
	 if(!empty($replies)){
       ...
       $is_fav  = $buff->isfav($user->id,$chield[$m]->id);
			...
			if($buff->post_type =="public"){
							  	if( $user->is_logged ){
									$childfootercnt='<a  style="cursor:pointer" onclick="childpopup('.$buff->post_id.','.$chield[$m]->id.')"><img class="icons" src="'.$C->SITE_URL.'static/images/icons/REPLY.png" title="Reply"/></a>
				...
				<div class="agree-list icon-ftr"><a href="" data-role="services" data-namespace="activities" data-action="'.($is_agree? 'disagree' : 'agree').'" data-value="'.htmlentities('{"activities_type":"public","activities_id":"'.$chield[$m]->id.'"}').'">'.($is_agree? '<img  width="30px" src="'.$C->SITE_URL.'static/images/icons/a2d.png"  title="Disagree"/>' : '<img width="30px" class="icons" src="'.$C->SITE_URL.'static/images/icons/a2d.png"  title="Agree"/>').'</a>'.$showagreebtn_btn.'</div>
               	<span class="reshare-list">'.$reshare_content.''.$resharecnt.'</span>';
								                          $childmark ='<div class="like-list icon-ftr">'.$mark_content.'</div>';

								}

				$footerhtml ='<div class="activity-footer meta-info col-xs-12 col-sm-12 col-md-12 col-lg-12">
		...
		if( count( $buff->post_attached ) > 0 ){ 
			...
			if( isset($buff->post_attached['file']) ){
				foreach($buff->post_attached['file'] as $k => $file){
					...
					$tpl->layout->inner_block->setVar('activity_attachments_files', ' <video controls width="100%" >
   ...
   '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 buzz-content event-list-blue-bg" style="padding:0px 0px 0px 10px;">
    ...
    <li>Status - <span class="txt-accepted">'.$st.'</span></li>
						...
						$tpl->layout->inner_block->setVar('activity_attachments_link_description', '<div class="btn-download-padding  event-list-blue-bg"><a href="'.$C->SITE_URL.'dashboard?pid='.$link->post_id.'"><input class="button-submit-results" type="button" name="download" value="Download Results"></a></div>');
							...
							$tpl->layout->inner_block->setVar('<div <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 event-list-blue-bg" style="padding:0px 10px 0px 6px;""><img class="icon-calander" src="'.$C->SITE_URL.'apps/events/static/images/icon-event-expired.png">&nbsp; <span class="event-expired">This event was no longer available.</span></div>');
