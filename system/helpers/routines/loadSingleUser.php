<?php
	...
	function loadSingleUser( $tpl, $params )
	{
		...
		$tpl->layout->block->setVar( 'single_user_avatar', '<a href="'.userlink($obj->username).'" class="author bizcard" data-userid="'.$obj->id.'"><img src="'.$C->STORAGE_URL.'avatars/thumbs1/'.(empty($obj->avatar)? $C->DEF_AVATAR_USER : $obj->avatar).'" alt="'.getThisUserCommunityName($obj).'" width="50" height="50" /></a>');
		...
		$tpl->layout->block->setVar( 'single_user_activity', '<a href="'.$C->SITE_URL.$obj->username.'/tab:friends/subtab:followers">'. $page->lang('usr_activity_count_small', array('#NUM_FOLLOWERS#'=>$network->format_num($obj->num_followers), '#NUM_POSTS#'=>$network->format_num($obj->num_posts))));
		if($user->is_logged){
