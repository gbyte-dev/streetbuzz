    ...
    {   
        ...
        if ($obj->attached > 0) {   
            if ($C->post_cache_is_activated) {  
                ...
                $r = $this->db2->query('SELECT id, type,post_id, data FROM ' . ($type == 'private' ? 'posts_pr_attachments' : 'posts_attachments') . ' WHERE post_id="' . $obj->id . '" LIMIT ' . $obj->attached, FALSE);   
...
<div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12 buzz-content" id="replace'.$poll[0]->poll_id.'">'; 
 foreach($pollper as $keys=>$vals){ 
    ...
    return $pollhtml;   
     ...
     //checking chield  
    public function pollhtml($parentid) 
    {   
...
$imagedes .= '<a id="'.$post_id.'" target="_blank" href="'.($C->STORAGE_URL.'tmp/'.$tmp->file_preview).'" class="lightbox-image image-thumb cboxElement"><img width="100%" alt="Image" src="'.$C->STORAGE_URL.'tmp/'.($tmp->file_preview).'" /></a>';
...
return $data;   
 ...
 $r      = $this->db2->query('SELECT message from  posts    
...
public function addviews($postid){  
    ...
    {   
       $adstype = 2;    
         $r   = $this->db2->query('SELECT ai.id,ai.sort_image,ai.big_image,ai.display_url,ai.contact_number,ai.ads_access_source,ai.whatsapp_number,ai.callnow_number FROM ads_tags as at   
    ...
    {   
        $adstype = 3;   
         $r   = $this->db2->query('SELECT ai.id,ai.sort_image,ai.big_image,ai.display_url,ai.contact_number,ai.ads_type,ai.ads_access_source,ai.whatsapp_number,ai.callnow_number FROM ads_tags as at   
    ...
    public function dialphonehtml($mobile,$adid,$postid){ 
        ...
        $new_html = "\n<a class='adsaction' data-customadid=".$adid." data-custompostid=".$postid."  style='font-size:12px;background-color:#075e54;font-size: 12px;   
    background-color: #25d366;  
    color: white;   
    border-color: #25d366;padding: 5px  ' href='".$display_url."'>Call Now</a>\n";  
         return $new_html;  
    }   
    ...
    background-color: #25d366;  
    color: white;   
    border-color: #25d366;padding: 5px'  href='".$display_url."'>WhatsApp</a>\n";   
         return $new_html;  
    }   
    public function knowmorehtml($url,$adid,$postid){ 
        $new_html = "\n <a class='adsaction' data-customadid=".$adid." data-custompostid=".$postid."  target= '_blank' style='font-size:12px;   
    color: #00BFFF; 
    border-color: #00BFFF;    border-radius: 13px;border: 1px solid;    
    padding: 5px;' href='".$url."'>Know More</a>\n";    
         return $new_html;  
