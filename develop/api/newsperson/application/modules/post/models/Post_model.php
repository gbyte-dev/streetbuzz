...
class Post_model extends My_Model {
    ...
    function top_post($data) {
       ...
       // $this->db->select("p.likes, p.comments, p.reshares, p.share_count, p.post_views_cnt");
        ...
        $this->db->select("if((p.posttype = 2), p.title,  SUBSTRING(p.message, 1, 144)) AS title");
    ...
    $this->db->save_queries = TRUE;
        ...
        if(!empty($district) || !empty($state) || !empty($country)) {
            ...
            if(!empty($state)) {
                $this->db->or_where ('LOWER(lm.location_state)', strtolower($state),NULL,FALSE);
            }
           
        }

        if(!empty($post_type)) {
            $this->db->where('p.posttype', $post_type);
        }
      ...
      //  $this->db->order_by('popularity', 'DESC');
        ...
        if($query->num_rows() > 0) {
            ...
            foreach ($results as $key => $result) {
                ...
                /* if($result["posttype"] != 2) {
                    $title = strip_tags($result["message"]); 
                    $result["title"] = mb_substr($title, 0, 144,'UTF-8');
                }
                */
    ...
    function get_event_post($user_id, $post_id) {
        ...
        if($query->num_rows() > 0) {
            ...
            if($event_id) {
                $this->db->select('id as event_id,admin_id as user_id,address,location,event_name,event_description,start_date,start_time,end_date,end_time,status');
                $this->db->from('events');
                ...
                if($query_event->num_rows() > 0) {
                    ...
                    $event_details['video_attachment'] = '';
    ...
    function more_news($data) {
        ...
        $this->db->select('IFNULL(u.cover,"") as coverimage', FALSE);
        $this->db->select("p.id AS postid, p.user_id AS postuserid, p.posttype, p.message, 
        p.mentioned, p.posttags, p.date, p.thumb , p.date_lastedit, p.date_lastcomment, p.group_name,
        p.parent_id, p.post_level, '' as `category`, 'public' AS `type`");
         $this->db->select("pa.video_url");  
         $this->db->select("pa.video_id");
        ...
        $this->db->select('IFNULL(p.thumb,"") as thumb', FALSE);
        $this->db->select('IFNULL(p.location,"") as location', FALSE);
       // $this->db->select("posts_attachments.*");
        //$this->db->select("posts_attachments.video_url");
       // $this->db->select("posts_attachments.video_id"); 
        
        $this->db->from("posts p");
        $this->db->join('users u', 'u.id = p.user_id');
        $this->db->join('posts_details pd', 'pd.post_id = p.id', 'LEFT');
        $this->db->join('posts_attachments pa', 'pa.post_id=p.id', 'LEFT');
        $this->db->where('p.user_id', $post_owner_id, FALSE);
        $this->db->where('p.id !=', $post_id);
     //   $this->db->where('p.post_level', 0);       //comment by ajay 
        $this->db->group_by('p.id');
        $this->db->order_by('p.date_lastcomment', 'DESC');
        $this->db->limit($page_size,$offset);
        $query = $this->db->get();
