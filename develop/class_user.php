<?php
	...
	class user
	{
		...
		public function login($login, $pass, $rememberme=FALSE)
		{
			...
			if(is_numeric($login)){
				$this->db2->query('SELECT id FROM users WHERE (phone_no="'.$login.'") AND password="'.$pass.'" AND active=1 LIMIT 1');

                           }else{
                                 	if( is_valid_email($login) ) {
                                 	 $this->db2->query('SELECT id FROM users WHERE (email="'.$login.'") AND password="'.$pass.'" AND active=1 LIMIT 1');

                                 	}else{
                                 	 $this->db2->query('SELECT id FROM users WHERE (username="'.$login.'") AND password="'.$pass.'" AND active=1 LIMIT 1');
			...
			$this->db2->query('update users set is_online=1 WHERE id="'.$this->id.'" ');
		...
		public function logout()
		{
			...
			$this->db2->query('UPDATE users set is_online=0   WHERE id="'.$this->id.'" ');
		...
		public function follow($whom_id, $how=TRUE)
		{
			...
			if( $how == TRUE ) {
				...
				send_push_notification($whom_id, $data);
			}
			else {
		...
		public function get_user_by_twitter_uid($uid)
		{
			$r	= $this->db2->query('SELECT * FROM users WHERE twitter_uid="'.$uid.'" LIMIT 1', FALSE);
		...
		public function get_admin_users()
		{
			...
			$r	= $this->db2->query('SELECT * FROM users WHERE is_network_admin="1"', FALSE);
		...
		public function save_search($search_string, $search_tab, $puser = '', $pgroup = '')
		{
			if(empty($search_string) || empty($search_tab)){
				return false;
			}
			
			$search_url = '/tab:'.$search_tab.'/s:'.$search_string;
			if(is_string($puser) && !empty($puser)){
				$search_url .= '/puser:'.$puser;
			}
			
			if(is_string($pgroup) && !empty($pgroup)){
				$search_url .= '/pgroup:'.$pgroup;
			}
			
			$search_key = md5($search_url);
			
			$r = $this->db2->query('SELECT 1 FROM searches WHERE search_key="'. $search_key .'" LIMIT 1');
			if( $o = $this->db2->fetch_object($r) ) {
				return false;
			}

			$this->db2->query('INSERT INTO searches(user_id, search_key, search_string, search_url, added_date) VALUES(
				'. (int) $this->id .', "'.$search_key.'", "'.$search_string.'", "'.$search_url.'", '. time() .'
			);');
			
			$this->get_saved_searches(TRUE);
		...
		{	
			...
			$data = array();
			$this->db2->query('SELECT id, search_url, search_string FROM searches WHERE user_id="'.$this->id.'" ORDER BY id DESC');
			while($tmp = $this->db2->fetch_object()) {
				$tmp->search_string		= stripslashes($tmp->search_string);
				$tmp->search_url	= 'search'.stripslashes($tmp->search_url);
				$data[$tmp->id]		= $tmp;
			}
			
			//$this->cache->set($cachekey, $data, $C->CACHE_EXPIRE);
			return $data;
		}
			...
			$pass		= $this->db2->escape($pass);
			if(is_numeric($login)){
				$user_id = $this->db2->fetch_field('SELECT id FROM users WHERE (phone_no="'.$login.'") AND password="'.$pass.'" AND active=1 LIMIT 1');

                }else{
										if( is_valid_email($login) ) {
										$user_id = $this->db2->fetch_field('SELECT id FROM users WHERE (email="'.$login.'") AND password="'.$pass.'" AND active=1 LIMIT 1');

										}else{
										$user_id = $this->db2->fetch_field('SELECT id FROM users WHERE (username="'.$login.'") AND password="'.$pass.'" AND active=1 LIMIT 1');
		...
		public function userpollanswerforsighup($userid,$pollid){

		$user =  $this->db2->fetch_field('SELECT ANSWER_ID  FROM post_poll_votes WHERE POLL_ID="'.$pollid.'" AND VOTER_USER_ID="'.$userid.'" ');
			return $user;
		
	}
	public function getuseridfacebook($login){
					if(is_numeric($login)){
						$user_id = $this->db2->fetch_field('SELECT id FROM users WHERE (phone_no="'.$login.'")  LIMIT 1');

					}else{
						$user_id = $this->db2->fetch_field('SELECT id FROM users WHERE (email="'.$login.'")  LIMIT 1');
