...
if( isset($_FILES['profile_avatar']) && is_uploaded_file($_FILES['profile_avatar']['tmp_name']) ) {
	...
	if($tab =='friends' && $subtab != 'ifollow' && $subtab !='followers' && $subtab !='incommon'){
	 ...
	 $folow_res           = $db2->query('SELECT whom FROM  users_followed as uf where who="'.$u->id.'"' );
	...
	$userdetails = $this->network->get_username($u->id);
	...
	$tpl->routine->load();
		$menu[] =array('url' => $u->username.'/tab:profileinfo', 	'css_class' => (($tab === 'profileinfo' || $tab==='updates' )? 'activeTab' :''), 	'title' =>'Profile info');


	$menu[] =array('url' => $u->username.'/tab:all', 	'css_class' => (($tab === 'all'  )? 'activeTab' :''), 	'title' => 'Buzzes');
	...
	/*		$menu[] = array('url' => $u->username.'/tab:likes', 	'css_class' => (($tab === 'likes')? 'activeTab' : ''), 	'title' => 'Likes' );
		 ...
		 //  $menu[] = array('url' => $u->username.'/tab:bookmarks', 	'css_class' => (($tab === 'bookmarks')? 'activeTab' : ''), 	'title' => 'Bookmarks' );
			...
			//$menu[] = array('url' => $u->username.'/tab:intraday/subtab:open', 	'css_class' => (($tab === 'intraday')? 'activeTab' : ''), 	'title' => 'Intraday' );
			$menu[] = array('url' => $u->username.'/tab:prediction', 	'css_class' => (($tab === 'prediction')? 'activeTab' : ''), 	'title' => 'Prediction' );
    ...
    $p =new post('public');
		 $myintradaycal       = $p->myintradaycorrectdatacalculation($u->id);
		 $totalintraday       = $p->totalintraday($u->id);
		 ...
		 $myintradayincal       = $p->myintradayincorrectdatacalculation($u->id);
		...
		$incorrectper = 0;
		  ...
		  $myopen       = $p->myintradayopendatacalculation($u->id);
		 $myopencnt          = $myopen->correctcnt;
		 if(!empty($myopencnt)){
		 $myopencntper          = round(($myopencnt/$totalcnt)*100,2);
		 }else{
		$myopencntper = 0;
	...
	switch($tab){
		...
		case 'likes':
			...
			if( !$is_posts_protected ){
                                               ...
                                               $menu = array( 
										array('url' => $u->username.'/tab:intraday/subtab:open', 'css_class' => (($subtab === 'open')? ' active' : ''), 		'title' =>'Open('.$myopencntper.'%)' ),
											   
											   
											   array('url' => $u->username.'/tab:intraday/subtab:correct', 	 'css_class' => (($subtab === 'correct')? ' active' : ''), 		'title' =>'Correct('.$correctper.'%)' ),
