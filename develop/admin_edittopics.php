<?php
	...
	$tpl->routine->load();
	$postid =$_GET['postid'];
    $topics_query        = $db2->query("select * from  sb_topics where id=$postid");
  $topicsres = $db2->fetch_object($topics_query);


   $categeory_masterquery        = $db2->query('select * from categeory_master ');
   $categeory_masterhtml ='';
   $categeoryid = $topicsres->topic_category;
   if($categeory_masterquery->field_count > 0){
       while($categeory_row = $db2->fetch_object($categeory_masterquery) ){
          if($categeoryid == $categeory_row->cat_id ){
             $categeory_masterhtml .="<option value=".$categeory_row->cat_id." selected>".$categeory_row->cat_name."</option>"; 
          }else{ 
     ...
     $attachlocation = $topicsres->topic_attach_location ;
   ...
   if($location_masterquery->field_count > 0){
       while($location_row = $db2->fetch_object($location_masterquery) ){
          if($attachlocation == $location_row->id ){
             $location_masterhtml .="<option value=".$location_row->id." selected>".$location_row->location."</option>"; 
          }else{
    ...
    $topic_attach_language = $topicsres->topic_attach_language ;
   ...
   if($language_masterquery->field_count > 0){
       while($language_row = $db2->fetch_object($language_masterquery) ){
          if($topic_attach_language == $language_row->id ){
               $language_masterhtml .="<option value=".$language_row->id." selected>".$language_row->language_name."</option>";
   	...
   	if(!empty($topicsres->valid_from)){
      $epoch1 = $topicsres->valid_from;
       $dt1 = new DateTime("@$epoch1");  // convert UNIX timestamp to PHP DateTime
       $validdate =  $dt1->format('m/d/Y'); // output = 2017-01-01 00:00:00
	}else{
	     $validdate ='';
	    
	}
	if(!empty($topicsres->valid_till)){
      $epoch2 = $topicsres->valid_till;
       $dt2 = new DateTime("@$epoch2");  // convert UNIX timestamp to PHP DateTime
       $validtilldate =  $dt2->format('m/d/Y'); // output = 2017-01-01 00:00:00
	}else{
	     $validtilldate ='';
    ...
    $datares    	.='<div class="col-md-6 content-bg">	
	 <div id="content-container">
		...
		<div id="center-container">
		       <h3>Edit Topic</h3><table class="form-container "><tbody>
			    <tr>
					<td class="field-title"><label for="network_name">Topic Literal<span style="
    color: red;
">*</span>:</label></td>
					<td><input type="text" id="customer_name" class="errormes" name="topic_name"  maxlength="50" autocomplete="off" value="'.$topicsres->topic_literal.'"></td>
				</tr>
					<tr>
...
">*</span>:</label></td>
            ...
            <td><textarea type="text"  id="description" class="errormes" style="width:500px;height:100px" name="description">'.$topicsres->topic_description.'</textarea></td>
        </tr>
			<tr>
...
">*</span>:</label></td>
  ...
  Browse File
</label><input type="file" id="files" name="files"   > <span class="txt-class">Image Size 1200 * 1200 < 5MB</span></td>		</tr>
<tr><td></td><td><img  width="100px"  src="'.$C->STORAGE_URL.'topics/'.$topicsres->topic_gallery.'"></td></tr>
...
">*</span>:</label></td>
            ...
            <td><textarea type="text"  id="hashtags"  class="errormes"  style="width:500px;height:100px" name="tags">'.$topicsres->topic_tags.'</textarea></td>
        </tr>
        	<tr>
    ...
    color: red;
">*</span>:</label></td>
					<td><input type="text" id="start_date"  class="errormes"  name="start_date"  maxlength="15" value="'.$validdate.'"> </td>
				</tr><tr>
					<td class="field-title"><label for="network_intro_txt">Valid Till<span style="
    color: red;
">*</span>:</label></td>
					<td><input type="text" id="end_date"  class="errormes"  name="end_date"  maxlength="50" autocomplete="off" value="'.$validtilldate.'"></td>
				</tr>
