<script>
		...
		$('#calendarall').fullCalendar({
			...
			viewRender: function (view) {
				...
				$('#calendarall').fullCalendar('option', 'contentHeight', h);
			},
			eventClick: function(data1, event, view) {
				var content = 'Invalid Key';
				$.ajax({
					url:siteurl + 'services/events/vieweventtabs',
					type: 'POST',
					data: {
						event: data1.id,
						group : $('#group_id').val(),
...
<div id="content_all" class="col-xs-12 content table " ></div>
