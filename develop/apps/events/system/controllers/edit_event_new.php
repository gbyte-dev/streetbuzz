<?php
	...
	if(!empty($page->params->gr)){
		$g	= $this->network->get_group_by_id($page->params->gr);
		if( ! $g ) {
	...
	$error = FALSE;
	$errmsg = '';
	$event_src = $db2->query('SELECT events.*, users.is_network_admin FROM ( events, users ) WHERE events.id="'.(int)$page->params->event.'" AND users.id = "'.(int)$this->user->id.'" LIMIT 1');
	...
	if( isset($_POST['title']) ) {
		...
		//$street_group   = substr($street_group);
		...
		if($error === FALSE){
			...
			if(!empty($event->id)){
				...
				$userdatav = $db2->query('SELECT pu.user_id,pu.post_id,pu.status FROM post_userbox AS pu

				WHERE  pu.post_id="'.$postidold[0].'" ');
				$userdatavstatus = $db2->query('SELECT pu.user_id,pu.post_id FROM post_userbox as pu
				...
				if($publish_now == 1){
					if( $this->user->id > 0 ) {
						...
						$db2->query('INSERT INTO posts SET user_id="'.$this->user->id.'",posttags="'.$street_count.'", group_id="'.$group_id.'",location="'.$address.'",date="'.time().'", date_lastcomment="'.time().'", attached="1"');
                        $pid = $db2->insert_id();
    ...
    <li><a href="'.$C->SITE_URL.'search/tab:location/s:'.$pid.'">'.$address.'</a></li>
    </ul>  
    </div>
    <!-- end : event location -->
   ...
   $content .='<!-- start : event url -->
    ...
    $content .='</div>
				...
				//$left = ($page->params->group)?(strlen($this->user->info->username)+strlen($g->title)+8):(strlen($this->user->info->username)+4);
					...
					$answer = (object)array(
						...
						while($om = $this->db2->fetch_object($userdatavstatus)) {

							if($om->user_id != $this->user->id ){
							$sql	=$this->db2->query('select user_id,newposts FROM  users_dashboard_tabs where user_id="'.$om->user_id.'" AND tab="notifications"');
							$userdashboard         =$this->db2->fetch_object($sql);
							if($userdashboard->user_id !=''){
								$newposts           =($userdashboard->newposts)+1;
								$db2->query('UPDATE users_dashboard_tabs SET newposts="'.$newposts.'" WHERE user_id="'.$om->user_id.'"  ');
							}else{
								$db2->query('INSERT INTO users_dashboard_tabs SET user_id="'.$om->user_id.'", tab="notifications",state=1,newposts=1  ');
								
							}
							
							        
							$db2->query('INSERT INTO notifications SET to_user_id="'.$om->user_id.'",notif_type="event",in_group_id="'.$group_id.'",from_user_id="'.$this->user->id.'", notif_object_type="user",date="'.time().'",post_id="'.$pid.'"  ');
	...
	ob_start();
	require_once($C->PLUGINS_DIR.'events/static/templates/blocks/edit_event_new.php');
	$edit_event_new = ob_get_clean();

	$this->setVar( 'main_content', $edit_event_new, 'replace');
