...
mysqli_select_db($connection,$dbname);
   ...
   $cur_date = date('Y-m-d');
    ...
    $postquery        = " select p.id,pt.tag_name from posts as p 
   ...
   if($post->num_rows > 0){
       while($post_results = mysqli_fetch_object($post)){
           $posttagstr = "#".$post_results->tag_name;
             $postid   = $post_results->id;
             $checkquery        = " select sbt.id,sbt.topic_literal,sbt.topic_description,sbt.topic_gallery from sb_topics as sbt
	     ...
	     if($check->num_rows > 0){
	        $topicres =  mysqli_fetch_object($check);
	        $topic_id = $topicres->id;
	        $updatequery        = " update posts set topic_id=$topic_id where id=$postid AND (topic_id IS NULL OR topic_id = 0 )";
	        $checkres  = mysqli_query($connection,$updatequery);
