<?php
	class notifier
	{
		...
		{//return true;
			...
			$this->tab_notif_users = array();
			if( $this->post->post_user->id != $this->user->id && $this->post->post_user->id > 0 ) {
				$n	= intval( $this->network->get_user_notif_rules($this->post->post_user->id)->ntf_me_on_post_like );
				if( $n != 0 ) {

					$this->_sendPost( 'ntf_me_on_post_like', $this->post->post_user->id );

					$this->_sendActivityNotification('like', $this->post->post_user->id, "buzz");

					$this->tab_notif_users[] = $this->post->post_user->id;
					$this->_sendTabNotification('notifications');
				}
			}
		...
		public function onGroupInvite( $group_id, & $group_members, & $invited_members )
		{
			...
			foreach($invited_members as $uid) {
				...
				$insert_values[] = '("'.(int) $group_id.'", "'.(int) $uid.'", "'. (int) $this->user->id.'", "'.$current_time.'")';
		...
		private function _sendActivityNotification($notifType, $whom_id = FALSE, $type ="buzz") {
			global $C;
			...
			if( $whom_id ){
				$this->db2->query('INSERT INTO active_notifications(group_id, from_user_id, to_user_id, post_id, noti_type, post_type, date) VALUES(
									"'.intval( $gid ).'", "'.intval($this->user->id).'", "'.intval($whom_id).'", "'.$this->db2->e($res_id).'", "'.$this->db2->e($notifType).'", "'.$this->db2->e($type).'",  "'.$current_time.'"
									)');
				return TRUE;
			}
		}
		...
		{ 
			...
			foreach( $send_to as $language => $to_users ){
				...
				$mail->AltBody  = $msgtxt; // optional, comment out and develop
