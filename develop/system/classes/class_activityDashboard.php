<?php
	class activityDashboard extends activity
	{
		public function setPostsQuery()
		{
			...
			switch($this->tab)
			{
				case 'all':
					...
					// break;
					//$this->query = 'SELECT b.id AS pid, p.*,sb.parent_id as parentid,"public" AS `type` FROM post_userbox b LEFT JOIN posts p ON p.id=b.post_id LEFT JOIN sb_chield sb ON sb.parent_id=b.post_id WHERE b.user_id="'.$this->user->id.'"';
					//$this->query_order = ' ORDER BY pid DESC ';
					//break;
					$this->query = 'SELECT b.id AS pid, p.*,"public" AS `type` FROM post_userbox b LEFT JOIN posts p ON p.id=b.post_id  WHERE b.user_id="'.$this->user->id.'" AND (p.post_level is null OR p.post_level= 0 )  ';
				...
				case 'bookmarks':
					...
					break;
                                case 'agree':
				    $this->query = 'SELECT p.*, p.id AS pid, "public" AS `type` FROM post_agree a, posts p WHERE a.post_id=p.id AND a.comment_id=0 AND a.user_id="'.$this->user->id.'" ';
				...
				case 'likes':
					$this->query = 'SELECT p.*, p.id AS pid, "public" AS `type` FROM post_likes a, posts p WHERE a.post_id=p.id AND a.comment_id=0 AND a.user_id="'.$this->user->id.'" ';
					$this->query_order = ' ORDER BY a.id DESC';
			}
			if($this->tab =='intraday' && $this->subtab =='' ){
				
				$this->query	= 'SELECT p.*
		                               FROM  posts  as p
		                              inner join post_dayfeel as pd on p.id= pd.post_id

		                                WHERE 	p.user_id="'.$this->user->id.'" group by pd.post_id ';
				...
				$this->query	= 'SELECT p.*
		                              ...
		                              inner join post_dayfeel as pd on p.id= pd.post_id
		                                ...
		                                WHERE 	p.user_id="'.$this->user->id.'" AND result= 1 group by pd.post_id ';
					$this->query_order = ' ORDER BY pd.id DESC ';
				...
				$this->query	= 'SELECT p.*
		                              ...
		                              inner join post_dayfeel as pd on p.id= pd.post_id
		                                ...
		                                WHERE 	p.user_id="'.$this->user->id.'" AND result= 0 group by pd.post_id ';
					$this->query_order = ' ORDER BY pd.id DESC ';
			}
			...
			if( empty( $this->query ) ){
				...
				if( is_string($tmp) ){
					...
					public function setPostsQueryView($postid,$userid)
		{
			...
			switch($this->tab)
			{
				case 'all':
					...
					//$this->query_order = ' ORDER BY pid DESC ';
					//break;
					$this->query = 'SELECT p.*,p.id as pid,"public" AS `type` FROM posts  p  WHERE p.user_id="'.$userid.'"  AND p.id <="'.$postid.'"  AND (p.post_level is null OR p.post_level= 0 )  ';
					$this->query_order = ' ORDER BY p.id DESC ';
					break;
		...
		{ 
			...
			{
				case 'all':
				$this->query	= 'SELECT COUNT(*) AS cnt FROM post_userbox b LEFT JOIN posts p ON p.id=b.post_id WHERE p.user_id<> 0 AND p.user_id<>"'.$this->user->id.'" AND b.user_id="'.$this->user->id.'" AND p.post_level is null AND  p.date_lastcomment>'.$this->lastactitivitydate;
