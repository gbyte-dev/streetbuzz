<?php
	...
	class postcomment
	{
		...
		public function delete_this_comment( $check_plugins = TRUE )
		{
			...
			$this->db2->query('DELETE FROM post_likes WHERE comment_id="'.$this->comment_id.'" AND user_id="'.$this->user->id.'" LIMIT 1');
			$this->error	= TRUE;
			
			if($this->post->post_type!='private') {
				$this->update_comment_count($this->post->post_id, -1);
			}


			$this->post->get_post_likes(TRUE);
			return TRUE;
		}
		...
		public function update_comment_count($post_id, $count = 1) { 
			...
			if($query->num_rows > 0){ 
				$this->db2->query('UPDATE posts_details SET comments=comments+('.$count.') WHERE post_id="'.$post_id.'"', FALSE); 
			} else if($count == 1) {
				$this->db2->query('INSERT INTO posts_details SET comments=1, post_id="' . $post_id . '"', FALSE);  
			}
		...
		public function is_comment_liked()
		{
			...
			if( !isset($this->post->post_likes['comment_'.$this->comment_id][$this->user->id]) ){
				return FALSE;
			}
		...
		public function like_comment($state=TRUE)
		{
			if( $this->post->error ) {
				return FALSE;
			}
			...
			$b	= $this->is_comment_liked();
			$u	= intval($this->user->id);
			if( $b && !$state ) {
				$this->db2->query('DELETE FROM post_likes WHERE user_id="'.$u.'" AND post_id="'.$this->post->post_id.'" AND comment_id="'.$this->comment_id.'" LIMIT 1', FALSE);
				//$this->db2->query('UPDATE posts_comments SET likes=likes-1 WHERE id="'.$this->comment_id.'" LIMIT 1', FALSE);
			}
			elseif( !$b && $state ) {
				$this->db2->query('INSERT INTO post_likes SET user_id="'.$u.'", post_id="'.$this->post->post_id.'", comment_id="'.$this->comment_id.'", date="'.time().'" ', FALSE);
				//$this->db2->query('UPDATE posts_comments SET likes=likes+1 WHERE id="'.$this->comment_id.'" LIMIT 1', FALSE);
			}
		
			$this->post->post_likes = array();
			$this->post->post_likes	= $this->post->get_post_likes(TRUE);
			return TRUE;
		}
		
		public function comment_could_be_liked()
		{
			if( !$this->comment_id ){
				return FALSE;
			}
			...
			if( $this->is_comment_liked($this->comment_id) ){
				return FALSE;
			}
