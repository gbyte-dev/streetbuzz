<?php
	...
	function networkbranding_logo_resize($source, $destination, $height)
	{
		...
		if( $C->IMAGE_MANIPULATION == "imagemagick_cli" ) {
			...
			$dstp	= imagecreatetruecolor($neww, $newh);
			 imagealphablending( $dstp, false );
			 imagesavealpha( $dstp, true );
    ...
    $C->COVER_SIZE4=50;
     $C->COVER_width=200;
    $C->COVER_SIZE_MOBILE_WIDTH = 50;
    $C->COVER_SIZE_MOBILE_HEIGHT = 50;
    if( ! file_exists($source) ) {
    return FALSE;
    }
    list($w, $h, $tp) = getimagesize($source);
    ...
    $fn5 = $C->STORAGE_DIR.'avatars/thumbs5/'.$fn;
    if( $C->IMAGE_MANIPULATION == "imagemagick_cli" ) {
    exec( $C->IM_CONVERT.' '.$source.' -gravity Center -resize '.$C->COVER_SIZE.'x -strip +repage '.$fn0 );
    exec( $C->IM_CONVERT.' '.$source.' -gravity Center -resize '.($w<$h?($C->COVER_SIZE1.'x'):('x'.$C->COVER_SIZE1)).' -crop '.$C->COVER_SIZE1.'x'.$C->COVER_SIZE1.'+0+0 -strip +repage '.$fn1 );
        exec( $C->IM_CONVERT.' '.$source.' -gravity Center -resize '.($w<$h?($C->COVER_SIZE2.'x'):('x'.$C->COVER_SIZE2)).' -crop '.$C->COVER_SIZE2.'x'.$C->COVER_SIZE2.'+0+0 -strip +repage '.$fn2 );
            exec( $C->IM_CONVERT.' '.$source.' -gravity Center -resize '.($w<$h?($C->COVER_SIZE3.'x'):('x'.$C->COVER_SIZE3)).' -crop '.$C->COVER_SIZE3.'x'.$C->COVER_SIZE3.'+0+0 -strip +repage '.$fn3 );
                exec( $C->IM_CONVERT.' '.$source.' -gravity Center -resize '.($w<$h?($C->COVER_SIZE4.'x'):('x'.$C->COVER_SIZE4)).' -crop '.$C->COVER_SIZE4.'x'.$C->COVER_SIZE4.'+0+0 -strip +repage '.$fn4 );
                    exec( $C->IM_CONVERT.' '.$source.' -gravity Center -resize '.($w<$h?($C->COVER_SIZE_MOBILE_WIDTH.'x'):('x'.$C->COVER_SIZE_MOBILE_HEIGHT)).' -crop '.$C->COVER_SIZE_MOBILE_WIDTH.'x'.$C->COVER_SIZE_MOBILE_HEIGHT.'+0+0 -strip +repage '.$fn5 );
                        ...
                        $dstp0 = imagecreatetruecolor($C->COVER_SIZE,$C->COVER_width);
                        $dstp1 = imagecreatetruecolor($C->COVER_SIZE1,$C->COVER_width);
                        $dstp2 = imagecreatetruecolor($C->COVER_SIZE2, $C->COVER_SIZE2);
                        $dstp3 = imagecreatetruecolor($C->COVER_SIZE3, $C->COVER_SIZE3);
                        $dstp4 = imagecreatetruecolor($C->COVER_SIZE4, $C->COVER_SIZE4);
                        $dstp5 = imagecreatetruecolor($C->COVER_SIZE_MOBILE_WIDTH, $C->COVER_SIZE_MOBILE_HEIGHT);
                        $dstp5_tmp = imagecreatetruecolor($C->COVER_SIZE_MOBILE_WIDTH, $C->COVER_SIZE_MOBILE_WIDTH);

               
                        $res0 = imagecopyresampled($dstp0, $srcp, 0, 0, 0, 0, $C->COVER_SIZE, $C->COVER_width, $w, $h);
                        $res1 = imagecopyresampled($dstp1, $srcp, 0, 0, $w>$h?round(($w-$h)/2):0, $w>$h?0:round(($h-$w)/2), $C->COVER_SIZE1, $C->COVER_width, min($w,$h), min($w,$h));
                        $res2 = imagecopyresampled($dstp2, $srcp, 0, 0, $w>$h?round(($w-$h)/2):0, $w>$h?0:round(($h-$w)/2), $C->COVER_SIZE2, $C->COVER_SIZE2, min($w,$h), min($w,$h));
                        $res3 = imagecopyresampled($dstp3, $srcp, 0, 0, $w>$h?round(($w-$h)/2):0, $w>$h?0:round(($h-$w)/2), $C->COVER_SIZE3, $C->COVER_SIZE3, min($w,$h), min($w,$h));
                        $res4 = imagecopyresampled($dstp4, $srcp, 0, 0, $w>$h?round(($w-$h)/2):0, $w>$h?0:round(($h-$w)/2), $C->COVER_SIZE4, $C->COVER_SIZE4, min($w,$h), min($w,$h));
    
                        $res5_tmp = imagecopyresampled($dstp5_tmp, $srcp, 0, 0, $w>$h?round(($w-$h)/2):0, $w>$h?0:round(($h-$w)/2), $C->COVER_SIZE_MOBILE_WIDTH, $C->COVER_SIZE_MOBILE_WIDTH, min($w,$h), min($w,$h));
                        $res5 = imagecopyresampled($dstp5, $dstp5_tmp, 0, 0, 0, round($C->COVER_SIZE_MOBILE_HEIGHT * 2), $C->COVER_SIZE_MOBILE_WIDTH, $C->COVER_SIZE_MOBILE_HEIGHT, 420, 60);
                        imagedestroy($srcp);
                        if( ! ($res0 && $res1 && $res2 && $res3 && $res4 && $res5 && $res5_tmp) ) {
                        imagedestroy($dstp0);
                        imagedestroy($dstp1);
	...
	function resize_link_image($source, $destination)
	{
		
	}
	
	function validate_image($image, $rename = true)
	{
		global $C;
		
		$error = FALSE;
		list($w, $h, $tp) = getimagesize($C->STORAGE_TMP_DIR.$image);
		...
		elseif( $w<$C->AVATAR_SIZE1 || $h<$C->AVATAR_SIZE1 ) {
			$error = TRUE;
		}
		...
		if($rename && !$error){
			...
			//@rename($C->STORAGE_TMP_DIR.$image, $C->STORAGE_TMP_DIR.$image.'.'.$ext);
			copy_attachment_videoimg($C->STORAGE_TMP_DIR.$image, $C->STORAGE_TMP_DIR.$image.'.'.$ext, $C->LINK_PREVIEW_IMAGE);
