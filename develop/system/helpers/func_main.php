...
// 		else return TRUE;
	...
	function checkPluginCompatability( $designer_for_ver )
	{
	 ...
	 function send_push_notification($user_id, $data) {
		$id = $data['id'];
		if($id == $user_id) {
			return true;
		}
		global $db2;
		$res = $db2->query('SELECT device_token FROM oauth_access_token WHERE user_id=' . $user_id . '  LIMIT 1');
		if ($db2->num_rows($res) > 0) {
			...
			if(!empty($device_token)) {
				...
				if(isset($data['username'])) {
					...
					$user_res = $db2->query('SELECT username FROM users WHERE id=' . $id . '  LIMIT 1');
					if ($db2->num_rows($user_res) > 0) {
						$obj = $db2->fetch_object($user_res);
				...
				if (isset($data['body']) && !empty($data['body']))  {
					...
					$data['body'] = mb_substr($data['body'], 0, 150,'UTF-8');
				} else {
					$data['body'] = '';
				}
				...
				switch($notification_type) {
					case 'follow':
						$data['title'] = $username. " is now following you";	
						$data['payload'] = array('id' => $id, 'type' => 'user');
						break;
					case 'comment':
						$data['title'] = $username. " commented on your post";
						$data['payload'] = array('id' => $data['postid'], 'type' => 'post');	
						unset($data['postid']);
						break;
				}
			...
			// $fields['notification']['android_channel_id']    = "test-channel";
				$fields['data']['largeIcon']    = 'large_icon';
				$fields['data']['smallIcon']    = 'small_icon';
				$fields['data']['subtitle']    = '';
				$fields['data']['tickerText']    = '';
				$fields['data']['vibrate']    = 1;   
     ...
     * @return array
     */
	function send_to_fcm($data) {
		
		global $C;
		$headers = array(
			'Content-Type:application/json',
			'Authorization:key=' . $C->FCM_API_KEY
		);

		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $C->FCM_API_URL);
	...
	/**
   * This function used to call newsperson api
   */
  function call_api($uri=null,$parameters=array()) {
	global $C;	
    $headers = array(
        'Content-Type: application/json',
    );
