<?php
	...
	if( isset($_POST['sbm']) ){ 
		...
		if( $tab == 'settings' && $subtab == 'main' && !$error ){
			...
			if( empty($errmsg) && isset($_POST['group_category']) && $_POST['group_category'] != $g->category_id ){
				$errmsg = $group->changeCategory( $_POST['group_category'], $sub_categories );
			}
	...
	//TEMPLATE START
	$tpl = new template( array('page_title' => $g->groupname. ' - ' .$C->SITE_TITLE, 'header_page_layout'=>'scs') );
	...
	if( $this->param('msg') == 'invited' ){
		$tpl->layout->setVar('main_content_placeholder', $tpl->designer->okMessage($this->lang('group_invite_members_hdr'), $this->lang('group_invite_members_txt') ) );
	}
	...
	if( $this->user->is_logged ){
		...
		$tpl->layout->block->setVar(
					'group_header_settings_button',
					'<div id="group" class="modal fade">jjfrdtu</div><a class="action-btn user-action add " onclick="joingrp('.$g->id.')" data-action="join" data-value="'.$g->id.'" ><span class="tooltip"><span>'.$this->lang('grp_toplnks_follow').'</span></a>'
			);
	...
	$tpl->layout->setVar( 'left_content', 	
			...
			'.$g->title.'
						...
						<ul>
						<li><img width="100%" src="'.$C->STORAGE_URL.'avatars/'. (empty($g->avatar)? $C->DEF_AVATAR_GROUP : $g->avatar).'" alt="'.$g->groupname.'"></li> 
				<li><div class="group-statistics"><br />
						<strong>'.$g->num_followers.'</strong> '.$this->lang('grp_tab_members').'<br />'.
						'<strong>'.$g->num_posts.'</strong> '.$this->lang('usrlist_numposts').'
					</div><br /></li>
				<li><div class="recent-visitors">'.
		...
		case 'settings':
				...
				if( $subtab == 'main' ){ 
					...
					if(count($sub_categories) > 0){
						$rows[] = $table->selectField($this->lang('group_settings_f_category'), 'group_category', $sub_categories, $g->category_id);
					}
					
					if( isset($C->USERS_TO_CREATE_SUBCATEGORIES) && ($this->user->info->is_network_admin == 1 || $C->USERS_TO_CREATE_SUBCATEGORIES == 1) ){
						$rows[] = $table->textField( '', '<a href="'.$C->SITE_URL.'group/addcategory/return:'.$g->groupname.'">'.$this->lang('newcat_create_new').'</a>' ); //name = alias = url
					}
					
					$rows[] = $table->textArea( $this->lang('group_settings_f_descr'), 'group_description', $g->about_me );
					...
					$rows[] = $table->textField( '', '<a href="'.$C->SITE_URL.$g->groupname.'/tab:settings/subtab:main/del:groupavatar">'.$this->lang('group_settings_avatar_remove').'</a>' );
					$rows[] = $table->submitButton( 'sbm', $this->lang('group_settings_f_btn') );
