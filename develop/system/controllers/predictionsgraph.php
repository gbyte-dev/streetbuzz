...
if($_POST['ticker']){
   $ticker = $_POST['ticker'];
	...
	$current_query            =$db2->query('SELECT 
	 market_data FROM assets as a
	 inner join asset_marketetails as am ON a.id = am.asset_id

	 WHERE a.ticker="'.$ticker.'" ORDER BY am.updated_date desc LIMIT 1');
	 $currentdata                       =$db2->fetch_object($current_query);
	 //1% added
	 $newmarketdata                     =((1/100)*$currentdata->market_data)+$currentdata->market_data;
	 /*end the current market data*/
	 /*get the historical symbol 
	 $historicalsym = $db2->query('SELECT histrolical_symbol FROM assets as a
	 WHERE a.ticker="'.$ticker.'" ');
	 $historicalsymdata                       =$db2->fetch_object($historicalsym);
	 $historicalsymbol                       = $historicalsymdata->histrolical_symbol;
	 */
	 

	 

    $historical_query            =$db2->query('SELECT price,price_date FROM asset_historical_data WHERE ticker="'.$ticker.'" AND price_date >=now()- interval 3 month order by price_date ASC ');
	while($histtoricaldata1[] = $db2->fetch_object($historical_query)){

	 } 


	 $histtoricaldata               =array_filter($histtoricaldata1);
	 $newarry                        =(object)array('price'=>$newmarketdata,'price_date'=>date('Y-m-d h:i:s'));
	 array_push($histtoricaldata,$newarry);
	 
	 $historicalcnt       =count($histtoricaldata);
	 $histrcnt           =$historicalcnt-1;
	 $datastr ='';
     for($i=0;$i<$historicalcnt;$i++){
		$year = substr($histtoricaldata[$i]->price_date,0,4);
		$month = (substr($histtoricaldata[$i]->price_date,5,2))-1;
		$date = substr($histtoricaldata[$i]->price_date,8,2);
		$price = number_format($histtoricaldata[$i]->price, 2, '.', '');
		if($i !=$histrcnt){
			$datastr .='[Date.UTC('.$year.', '.$month.', '.$date.'),'.$price.'],';
		}else{
			$datastr .='[Date.UTC('.$year.', '.$month.', '.$date.'),'.$price.']';
	...
	//calculations for set the end date
	$before3months     =  date("m",strtotime("-3 Months"));
	$before3monthsreduce =  $before3months-1;
	$before3monthsyear =  date("Y",strtotime("-3 Months"));
	$after3months      =  date("m",strtotime("+3 Months"));
	$after3monthsreduce =  $after3months-1;
	$after3monthsyear  =  date("Y",strtotime("+3 Months"));
	$after3monthsnoofdays =cal_days_in_month(CAL_GREGORIAN,$after3months,$after3monthsyear);
	//user availble amount and user level
	$userlevel_query            =$db2->query('SELECT Level,NotionalAmount FROM user_predict_details WHERE User_id ="'.$this->user->id.'" ');
	$userdata                   =$db2->fetch_object($userlevel_query);
	if(!empty($userdata)){
	$userlevel                  =$userdata->Level;
	$availableamount            =$userdata->NotionalAmount;
	}else{
	$userlevel                  ='';
	$availableamount            =0;
...
<!-- Start : Steps -->
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 prediction-popup-pages-bor">
<ul class="list-inline prediction-popup-pages">
<li class="currentpage">Step 1</li>
<li>Step 2</li>
<li>Step 3</li>
<li>Step 4</li>
</ul>
...
<!--/ End : Steps -->
          ...
          <h4 class="modal-title">Drag End Node to Predict Price & End Time ($<?php echo strtoupper($ticker);?>)</h4>
        </div>
        <div class="modal-body">
        <div class="container prediction-graph"></div>
		<!--<div id="drag"></div>
		<div id="drop"></div>-->
		<input type="hidden" id="droggable" value='' >
		<input type="hidden" id="finalamnt" value='' >
		<input type="hidden" id="enddate" value='' >
		<input type="hidden" id="currentmarket" value='<?php echo $currentdata->market_data;?>'>
		<input type="hidden" id="predicttype" value=''>


          
        </div>
		<div class="modal-footer">
       <button type="button" class="btn btn-primary btn-lg  btn-blue show-modal" data-toggle="modal" data-target="#next" id="next-process" disabled>Next</button>
      </div>
...
<!-- Modal for next reason filling -->
<div id="next" class="modal fade" data-backdrop="static" data-keyboard="false">
...
<ul class="list-inline prediction-popup-pages">
<li>Step 1</li>
<li class="currentpage">Step 2</li>
<li>Step 3</li>
<li>Step 4</li>
</ul>
</div>
<!--/ End : Steps -->
        ...
        <button type="button" class="close" >&times;</button>
        <h4 class="modal-title">Review Predicted Price & End Date. Provide reason for change</h4>
      </div>
      <div class="modal-body">
   ...
   <div class="row">
	      <div class="col-md-4">
			  <span class="titleModal">Predicted Price:</span><span><input type="text" class="form-control" id="predicted_price" value="" name="predict_price" ></span>
		  </div>
<div id="predictprice-error"class="notifyjs-container" style="top: 65px; left: 21px; overflow: hidden; display: hidden;"><div class="notifyjs-bootstrap-base notifyjs-bootstrap-error">
<span data-notify-text="" class="notifyjs-text">This field is required</span>
</div></div>		 

 <div class="col-md-4">
				<span class="titleModal">Price changed:</span><ul class="list-inline"><li><img id="my_image" src=""/></li><li><span class="dataValues" id="pricechanged"></span></li></ul>
          
		  </div>
		  <div class="col-md-4" >
			 
                <span class="titleModal">End Date:</span><span>

                <ul class="list-inline input-group">
                <li>
            <input  id="graphdate" class="form-control" type="text" readonly value="" name="enddate" />
</li>
<li>
    <script>
    $("#graphdate").datepicker(
	{minDate: 1,dateFormat: 'yy-mm-dd',showOn: 'button',
        ...
        buttonImage: 'http://jqueryui.com/resources/demos/datepicker/images/calendar.gif'}
		  ...
		  <div class="col-md-12">
		      	<span class="titleModal">Reason:</span><input type="text" value="" class="form-control" id="reason" name="reason">
          </div>
		  <div id="reason-error"class="notifyjs-container error-alert"><div class="notifyjs-bootstrap-base notifyjs-bootstrap-error">
            <span data-notify-text="" class="notifyjs-text">This field is required</span>
         </div></div>
</div>
      ...
      <div class="modal-footer">
		        <button  type="button" class="btn btn-default btn-primary btn-blue" data-toggle="modal" data-target="#simulate" id="resaonsfill"  >Next</button>
  ...
  <!-- Modal for simulate or not -->
  <div class="modal fade" id="simulate" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
       		<form method="POST" action="<?php echo $C->SITE_URL;?>predictionsinsert">
...
<li>Step 1</li>
<li>Step 2</li>
<li class="currentpage">Step 3</li>
<li>Step 4</li>
</ul>
</div>
<!--/ End : Steps -->	
          ...
          <button type="button" class="close" >&times;</button>
          <h4 class="modal-title">Should this prediction count towards your Rating?</h4>
        </div>
        <div class="modal-body">
		     <div class="row">
			     <div >
			 
	<label><input type="radio" name="simulatetype" id="yes" value="yes" checked> Yes</label>

                 </div>
				 <div >
			 
			        	<label><input type="radio" name="simulatetype" id="no" value="no"> No</label>

                 </div>
			<input type="hidden" name="predicted_price" value="" id="predicted_price_final" >
			<input type="hidden" name="predicted_graph_date" value="" id="graphdate_final" >
			<input type="hidden" name="predicted_reason" value="" id="reason_final" >
			<input type="hidden" name="predicted_bidding" value="" id="bidding_final" >
			<input type="hidden" name="predicted_currentprice" value="<?php echo $currentdata->market_data;?>"  >
			<input type="hidden" name="predicted_message" value="<?php echo $ticker;?>"  >
			<input type="hidden" name="user_type" value="<?php echo $userlevel;?>"  >

			<input type="hidden" name="user_available_balance" value="<?php echo $availableamount;?>"  >
	...
	<?php if($userlevel =="EXPERT" || $userlevel =="MAESTRO"){?>

        <div class="modal-footer" id="replace">
		   <button type="button" class="btn btn-primary btn-blue"  data-toggle="modal" data-target="#afterreason" id="avail_balance" >Next</button>

	</div><?php }else{ ?>
	<div class='modal-footer' id='replace'><input  type='submit' class='btn btn-primary btn-blue'  value='Done'></div>
...
<!-- Modal for after filling reasons -->
  <div class="modal fade" id="afterreason" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
	   <form method="POST" action="<?php echo $C->SITE_URL;?>predictionsinsert">
...
<li>Step 2</li>
<li>Step 3</li>
<li class="currentpage">Step 4</li>
</ul>
</div>
<!--/ End : Steps -->
          ...
          <button type="button" class="close" >&times;</button>
          <h4 class="modal-title">How much amount you want to placed on this prediction?</h4>
        </div>
        <div class="modal-body">
		     <div class="row">
			     <div class="col-md-4" >
			 
			         <span><input  id="bidding" class="form-control" type="text" name="bidding_amount"  /> </span>


		      </div>
			   <div class="col-md-4" >
			 
		      	<span class="titleModal">Available Balance:</span>


		      </div>
			    <div class="col-md-4" >
			 
		      	<span  id="avalablebalance"><?php echo $availableamount;?></span>


		      </div>
			   <input type="hidden" name="simulatetype" value="yes" >
			  <input type="hidden" name="predicted_price" value="" id="predicted_price_final_change" >
			<input type="hidden" name="predicted_graph_date" value="" id="graphdate_final_change" >
			<input type="hidden" name="predicted_reason" value="" id="reason_final_change" >
			<input type="hidden" name="predicted_bidding" value="" id="bidding_final_change" >
			<input type="hidden" name="predicted_currentprice" value="<?php echo $currentdata->market_data;?>"  >
			<input type="hidden" name="predicted_message" value="<?php echo $ticker;?>"  >
			<input type="hidden" name="user_type" value="<?php echo $userlevel;?>"  >

			<input type="hidden" name="user_available_balance" value="<?php echo $availableamount;?>"  >
			  <div id="balance-error"class="notifyjs-container" style="top: 47px; left: 21px; overflow: hidden; display: hidden;"><div class="notifyjs-bootstrap-base notifyjs-bootstrap-error">
            <span data-notify-text="" class="notifyjs-text" id="availerror">Amount put on prediction   should be lessthan Available balance</span>
        ...
        <div class="modal-footer">
          <input type="submit" class="btn btn-primary btn-blue" id="predict_next" value="Done"  ></button>
        </div>
      </div>
...
<!-- synchronus javascript solving issues -->
<script>
/*
...
resetZoom:"Reset zoom",resetZoomTitle:"Reset zoom level 1:1",thousandsSep:" "},global:{useUTC:!0,VMLRadialGradientURL:"http://code.highcharts.com/4.2.4/gfx/vml-radial-gradient.png"},chart:{borderColor:"#4572A7",borderRadius:0,defaultSeriesType:"line",ignoreHiddenSeries:!0,spacing:[10,10,15,10],backgroundColor:"#FFFFFF",plotBorderColor:"#C0C0C0",resetZoomButton:{theme:{zIndex:20},position:{align:"right",x:-10,y:10}}},title:{text:"",
...
shadow:!0,snap:Lb?25:10,style:{color:"#333333",cursor:"default",fontSize:"12px",padding:"8px",pointerEvents:"none",whiteSpace:"nowrap"}},credits:{enabled:!0,position:{align:"right",x:-10,verticalAlign:"bottom",y:-5},style:{cursor:"pointer",color:"#909090",fontSize:"9px"}}};var ba=S.plotOptions,ea=ba.line;Eb();ja.prototype={parsers:[{regex:/rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/,parse:function(a){return[F(a[1]),
...
b,c){
	...
	this.applyOptions(b,c);this.pointAttr={};if(a.options.colorByPoint&&(b=a.options.colors||a.chart.options.colors,this.color=this.color||b[a.colorCounter++],a.colorCounter===b.length))a.colorCounter=0;a.chart.pointCount++;return this},applyOptions:function(a,b){var c=this.series,d=c.options.pointValKey||c.pointValKey,a=Ia.prototype.optionsToObject.call(this,a);s(this,a);this.options=this.options?s(this.options,a):a;if(d)this.y=this[d];this.isNull=this.y===null;
...
(function (factory) {
    ...
    Highcharts.Chart.prototype.callbacks.push(function (chart) {
        ...
        function getNewPos(e) {
			...
			var draggablepoint  = series.processedXData[histrcnt];
			$("#droggable").val(draggablepoint);

			var dragarry = [];
			if(draggablepoint <= newX){
				//console.log('yes');
            ...
            each(series.points, function (point) {
                ...
                if (!point.handle) {
                    point.handle = series.chart.renderer.path(path)
                        .attr({
                            fill: options.dragHandleFill || 'rgba(34,04,0,0.5)',
	...
	<script>
	var predictdata = [ <?php echo $datastr;?>];
	var histrcnt   ="<?php echo $histrcnt; ?>";
    ...
    $('.container').highcharts({
        ...
        xAxis: {
			...
			tickInterval: 30 * 24 * 3600 * 1000,
                min: Date.UTC(<?php echo $before3monthsyear; ?>, <?php echo $before3monthsreduce;?>,1),
                max: Date.UTC(<?php echo $after3monthsyear;?> ,<?php echo $after3monthsreduce;?>, <?php echo $after3monthsnoofdays; ?>),
        ...
        plotOptions: {
            ...
            point: {
                events: {
                    ...
                    drag: function (e) {
						var dragpoint = $("#droggable").val();
						if(this.category >= dragpoint ){
							
						}else{
							return false;
						}
						$("#finalamnt").val(Highcharts.numberFormat(this.y, 2));
						$("#next-process").prop("disabled",false);
 ...
 function dateconvert(){
		min =parseInt($("#droggable").val());
                        var date = new Date(min);
                        var off = date.getTimezoneOffset() * 60 * 1000;
                        var newStamp = (min + off);
                        var newDate = new Date(newStamp);
						$("#enddate").val(newDate);
	}
	
	$("#resaonsfill").click(function(){
		if($("#predicted_price").val() ==''){
		$("#predictprice-error").css("display","block");
		$("#predictprice-error").delay(5000).fadeOut(400);
		  return false;
		}
		if($("#reason").val() ==''){
		$("#reason-error").css("display","block");
		$("#reason-error").delay(5000).fadeOut(400);
		  return false;
		}
		var pridected_price_graph  = $("#predicted_price").val();
		var pridected_graph_date   = $("#graphdate").val();
		var pridected_reason       = $("#reason").val();
		var pridected_bidding       = 0;
		$("#predicted_price_final").val(pridected_price_graph);
		$("#graphdate_final").val(pridected_graph_date);
		$("#reason_final").val(pridected_reason);
		$("#bidding_final").val(pridected_bidding);
		$("#predicted_price_final_change").val(pridected_price_graph);
		$("#graphdate_final_change").val(pridected_graph_date);
		$("#reason_final_change").val(pridected_reason);
		$("#bidding_final_change").val(pridected_bidding);
		$("#next").hide();
		
	});
	$("#next-process").click(function(){
		$("#graph-chat").fadeOut();
		var currentmarket = $("#currentmarket").val();
		var finalamnt = $("#finalamnt").val(); 
		var finalamnttrim =finalamnt.replace(' ','');
		$("#predicted_price").val(finalamnttrim);
		var crrenttrim =currentmarket.replace(' ','');

		var pricechanged  =(finalamnt);
		var enddate       =$("#enddate").val();
		var endateval      =enddate.split(' ');
		var monthname      =endateval[1];
		var graphdate      =endateval[2];
		var graphyear      =endateval[3];
		var graphtime      =endateval[4];
		var monthnumber   = (new Date(Date.parse(monthname +" 1,"+graphyear)).getMonth()+1);
		var graphdate     = graphyear+'-'+monthnumber+'-'+graphdate;
		$("#graphdate").val(graphdate);

		


		              
		$("#pricechanged").html((((Number(finalamnttrim)-Number(crrenttrim))/Number(crrenttrim))*100).toFixed(2)+'%');
		var str1 = (((Number(finalamnttrim)-Number(crrenttrim))/Number(crrenttrim))*100).toFixed(2);
		var searchstr        =str1.indexOf('-');
		if(searchstr =="-1"){
			$("#predicttype").val('up');

			$('#my_image').attr('src','<?php echo $C->SITE_URL;?>static/images/icons/up-arrow-prediction.png');


		}else{
			$("#predicttype").val('down');
			$('#my_image').attr('src','<?php echo $C->SITE_URL;?>static/images/icons/down-arrow-prediction.png');
	...
	$("#reason").keyup(function(){
		var dta = $(this).val();
		if(dta !=''){
		  var predict = 	$("#predicttype").val();
		 
			$.ajax({
			type: "POST",
			url:"<?php  echo $C->SITE_URL;?>autocomplete",
			data:{input:dta,predict:predict},
	...
	$("#predicted_price").click(function(){
		$("#predictprice-error").css("display","none");
	});
	$("#reason").click(function(){
		$("#reason-error").css("display","none");
	});
	$("#predicted_price").keyup(function(){
		
		var finalamnt    =$(this).val();
		var currentmarket = $("#currentmarket").val();
		var crrenttrim =currentmarket.replace(' ','');
	

		$("#pricechanged").html((((Number(finalamnt)-Number(crrenttrim))/Number(crrenttrim))*100).toFixed(2)+'%');
		var str1 = (((Number(finalamnt)-Number(crrenttrim))/Number(crrenttrim))*100).toFixed(2);
		var searchstr        =str1.indexOf('-');
		if(searchstr =="-1"){
			$('#my_image').attr('src','<?php echo $C->SITE_URL;?>static/images/icons/up-arrow-prediction.png');
	...
	$("#bidding,#predicted_price").keypress(function(e){
     if (((e.which < 46 || e.which > 57))) {
		 //mozilla backspce is not working.
		 if(e.which ==8){
			 return true;
		 }else{
	...
	$("#bidding").keyup(function(){
		var availamnt          =(<?php echo $availableamount;?>)-(Number($(this).val()));
		$("#avalablebalance").html(availamnt);

		if(availamnt < 0){
			$("#availerror").html('Amount put on prediction should be lessthan Available balance');
			$("#balance-error").css("display","block");
			$("#balance-error").delay(5000).fadeOut(400);
			return false;
	...
	$("#predict_next").click(function(){
		if($("#bidding").val() ==''){
			$("#balance-error").css("display","block");
			$("#availerror").html('Field is required');
			$("#balance-error").delay(5000).fadeOut(400);
			return false;
			
		}
		var availamnt          =(<?php echo $availableamount;?>)-(Number($("#bidding").val()));
		...
		var pridected_graph_date   = $("#graphdate").val();
		var pridected_reason       = $("#reason").val();
		var pridected_bidding       = $("#bidding").val();
		$("#predicted_price_final").val(pridected_price_graph);
		$("#graphdate_final").val(pridected_graph_date);
		$("#reason_final").val(pridected_reason);
		$("#bidding_final").val(pridected_bidding);
...
$("#no").click(function(){
	$("#replace").replaceWith("<div class='modal-footer' id='replace'><input  type='submit' class='btn btn-primary btn-blue'  value='Done'></div>");
 
});
$("#yes").click(function(){
	if(user_level =="EXPERT" || user_level =="MAESTRO" ){
	$("#replace").replaceWith(" <div class='modal-footer' id='replace'><button type='button' class='btn btn-primary btn-blue' data-toggle='modal' data-target='#afterreason' id='avail_balance'>Next</div>");
	}else{
		$("#replace").replaceWith("<div class='modal-footer' id='replace'><input  type='submit' class='btn btn-primary btn-blue'  value='Done'></div>");
		
	}
});
$("#avail_balance").click(function(){
	$("#simulate").hide();
...
<script>
  ...
  <style>
#country-list{float:left;list-style:none;margin-top: -10px;padding:0;}
#country-list li{padding: 10px; background:#FAFAFA;border-bottom:#F0F0F0 1px solid;height: 32px;}
...
.error-alert {
top: 165px; left: 6px; overflow: hidden; display: hidden;
}
@media screen and (max-width: 480px) {
.error-alert {
top: 272px; left: 6px; overflow: hidden; display: hidden;
}	
}
</style>
