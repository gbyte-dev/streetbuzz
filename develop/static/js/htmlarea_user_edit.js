...
var usertxt = function () {
    ...
    function insertUserLink(el, editor) {
        ...
        if (document.selection) {
            ...
            if (startPos == endPos) {
                ...
                range.moveEnd("character", offsetToRangeCharacterMove($(editor)[1], endPos));
                range.moveStart("character", startCharMove);
            }
            range.text = '@' + alias;
        } else {
    ...
    function highlighter(el) {
        ...
        if ($(el).parents('#usertxtedit').length > 0) {
            $(el).parents('#usertxtedit').find('.textarea-highlighter').html(cnt);
        } else {
