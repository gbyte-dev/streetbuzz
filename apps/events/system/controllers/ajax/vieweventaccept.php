			...
			if($user->id == $event->admin_id || $db2->num_rows($us_data) > 0){
				if($event->display_type == 'group'){
					...
					if(empty($_POST['username'])){
						$data .='<div class="row links"><a  title="Edit"  onclick="myfunction2('.$event->id.')" class="btn btn-primary edev" rel="'.$C->SITE_URL . $user->info->username.'/tab:events/action:edit_event/event:'.$event->id.'"><span>Edit</span></a>';
						if($event->status == 1){
