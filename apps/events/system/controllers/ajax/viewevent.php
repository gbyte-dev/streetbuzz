	...
	if( isset($_POST['event']) ){
		...
		while ($event = $db2->fetch_object($event_src)) {
			if($event->event_type=='major'){
				...
				$title= '<h2  style="padding-left:10px;"><b>'.$event->event_name.'</b></h2>';
			}
			$user_info= $this->network->get_user_by_id($event->admin_id);
			$data = '<div class="popup_event">'.$title;
			$data .= '<div class="col-md-12"><label>Author</label>:<span>'.htmlentities($user_info->username, ENT_COMPAT, 'UTF-8').'</span> </div>';
			if(!empty($event->group_id)){
				$data .= '<div class="col-md-12"><label>Group</label>:<span>'.$event->gtitle.'</span> </div>';
			}
			//$data .= '<div class="row"><label>Title</label>:<span>'.strip_tags(htmlentities($event->event_name, ENT_COMPAT, 'UTF-8')).'</span> </div>';
			$data .= '<div class="col-md-12"><label>Start Time</label>:<span>'.date('M d, Y', strtotime($event->start_date)).' '.date('h:i A', strtotime($event->start_time)).'</span> </div>';
			$data .= '<div class="col-md-12"><label>End Time</label>:<span>'.date('M d, Y', strtotime($event->end_date)).' '.date('h:i A', strtotime($event->end_time)).'</span> </div>';
			if(!empty($event->address)){
				...
				$data .= '<div class="col-md-12"><label>Address</label>:<span>'.htmlentities($event->address, ENT_COMPAT, 'UTF-8').'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a style="color:darkblue; font-weight:bold;" onclick="'.$window_pop.'" href="#">view map</a></span> </div>';
			}
			$data .= '<div class="col-md-12"><label>Description</label>:<span>'.htmlentities($event->event_description, ENT_COMPAT, 'UTF-8').'</span> </div>';
			$data .= '<div class="col-md-12"><label>Type</label>:<span>'.htmlentities($event->event_type, ENT_COMPAT, 'UTF-8').'</span> </div>';
			if($event->status == 1){
				$data .= '<div class="col-md-12"><label>Status</label>:<span><b>Active</b></span> </div>';
			}elseif($event->status == 2){
				$data .= '<div class="col-md-12"><label>Status</label>:<span><b>Cancelled</b></span> </div>';
			}else{
				$data .= '<div class="col-md-12"><label>Status</label>:<span><b>Expired</b></span> </div>';
			}
			$data.=' <div class="col-md-12"><label><h3>Attachments:</h3></label>:<span>';
			$data_attach = '';
			...
			if($user->id == $event->admin_id || $db2->num_rows($us_data) > 0){
				if($event->display_type == 'group'){
					$data .='<div class="col-md-12 event-btn links"><a  title="Edit" class="btn btn-primary btn-sm" href="'.$C->SITE_URL . $user->info->username.'/tab:events/action:edit_event/event:'.$event->id.'/gr:'.$event->group_id.'">Edit</a>';
					if($event->status == 1){
						$data .='<a class="btn btn-primary btn-sm" href="'.$C->SITE_URL.'plugin/events/cancel_event/event:'.$event->id.'/group:'.$event->group_id.'" title="Cancel Event"  onclick="'.$cancel.'" >Cancel Event</a></div>';
					}
				}else{
					if(empty($_POST['username'])){
						$data .='<div class="col-md-12 event-btn links"><a  title="Edit" class="btn btn-primary btn-sm" href="'.$C->SITE_URL . $user->info->username.'/tab:events/action:edit_event/event:'.$event->id.'">Edit</a>';
						if($event->status == 1){
							$data .='<a class="btn btn-primary btn-sm" href="'.$C->SITE_URL.'plugin/events/cancel_event/event:'.$event->id.'" title="Cancel Event" onclick="'.$cancel.'" >Cancel Event</a></div>';
						}
					}else{
						$data .='<div class="col-md-12 event-btn links"><a  title="Edit" class="btn btn-primary btn-sm" href="'.$C->SITE_URL . $user->info->username.'/tab:events/action:edit_event/event:'.$event->id.'/username:'.$_POST['username'].'">Edit</a>';
						if($event->status == 1){
							$data .='<a class="btn btn-primary btn-sm" href="'.$C->SITE_URL.'plugin/events/cancel_event/event:'.$event->id.'/username:'.$_POST['username'].'" title="Cancel Event" onclick="'.$cancel.'" >Cancel Event</a></div>';
						}
					}
				}
			}
			...
			if(!empty($event->group_id)){
				...
				if(!empty($role_selct)){
					...
					$data.='<div class="col-md-12"><label>Event Resources </label>:<span>'.roles_resource_list($event->group_id, $event->id).'</span></div>';
				}
			}
		}
