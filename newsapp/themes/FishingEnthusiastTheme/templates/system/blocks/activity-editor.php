<div class="status-editor" style="margin:20px 0px 0px">
	<div class="user-status-field-container" data-type="{%postform_type%}">
		<div class="data-content-placeholder status-editor-container">
			<div class="status-editor-container" style="padding: 0px;">
				{%editor_textarea%}
				{%editor_character_limit%}
			</div>
			
			<div class="attachments uploads" id="0f768e1c400608">
				<div class="images"></div>
				<div class="links"></div>
				<div class="files"></div>
				{%editor_attachment_uploads%}
			</div>

			{%editor_attachment_options%}
			
			{%editor_btn_placeholder%}
			
			
			<div class="clear"></div>
		</div>
	</div>

	<div class="clear"></div>
</div>	