...
if($sss){
	...
	$rehtml = $buff->pollhtml($post_id);
	$rehtml .= '<style>
	
	
	a.lightbox-image.image-thumb.cboxElement'.$post_id.'.cboxElement{
    display: none;
}
	.btn-white {
		border-color: #0084B4;
...
if($_GET['action'] =="textedit"){
	...
	$db2		= & $this->network->db2;
	$time= time();
	$db2->query('UPDATE posts set message="'.$postmessage.'", date="'.$time.'" Where id="'.$post_id.'"');
	...
	$finalcon='<p><a href="'.$C->SITE_URL.'/view/post:'.$post_id.'" class="permlink">'.$date.'<span class="glyphicon glyphicon-link"></span></a></p><pre>'.$postmessage.'</pre>  <p></p><p></p>';
...
$db2->query('INSERT INTO `posts_attachments`(`post_id`, `type`, `data`,  `content`) VALUES ('.$poll_id.',\'Image\',\''.$db2->escape(serialize($ii)).'\',\''.$imagecaption.'\')');
    ...
    <input type='button' class='btn btn-xs btn-white active' id='userpoll' value='+Add Users' />
      ...
      <input type='file' name='file' class='btn btn-xs btn-white active' id='file' value='' accept='image/png,image/jpeg,image/jpg' onchange='return validateInputfile()'  required/>
