...
<div class="login-form" id="login-panel">
	<form action="{%home_form_action%}" method="post">
		...
		<input type="text" class="text" id="email" name="email" data-status="focus" value="<?php if(isset($_COOKIE['username'])){ echo $_COOKIE['username'];} ?>">
	
	
		<label for="password"><?= $this->page->lang('signin_form_password') ?></label>
		<input type="password" class="text" id="password" name="password" value="<?php if(isset($_COOKIE['userpassword'])){ echo $_COOKIE['userpassword'];} ?>">
		...
		<div class="registration-buttons">
			...
			<span class="remember-me">
				<input type="checkbox" name="rememberme" id="rememberme"  value="1" <?php if(isset($_COOKIE['userpassword'])){  echo "checked";} ?> ><label for="rememberme" ><?= $this->page->lang('signin_form_rem') ?></label>
			</span> 
			<div class="clear"></div>
		</div>
