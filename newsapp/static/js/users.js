...
var Users = function () {
    ...
    function loveSuccess(response, context) {
		
		if(response.status!='ERROR'){
			$(".love").replaceWith(response.html);
		}else{
			STX.showMessage(response.message, "error");
		}
	}
	function showlovesSuccess(response, context) {
	...
	function unloveSuccess(response, context) {
	
		if(response.status!='ERROR'){
			$(".unlove").replaceWith(response.html);
		}else{
			STX.showMessage(response.message, "error");
		}
	}
    ...
    function userBizcard1() {
        $('body').append(container);
        $('.bizcard1').live('mouseover', function () {
            if (!$(this).data('hoverIntentAttached')) {
                $(this).data('hoverIntentAttached', true);
                $(this).hoverIntent(
                    function () {
                        ...
                        var reposition = { left: pos.left-300 + width + 'px', top: pos.top + 20 + 'px' };
    ...
    return {
       ...
       love: function(el, value, event) {
			...
			Services.invoke(args, loveSuccess, commandFail, el);
        },
		 showloves: function(el, value, event) {
			...
			Services.invoke(args, showlovesSuccess, commandFail, el);
        },
		 unlove: function(el, value, event) {
        	var args = {
					//type: 'post',
					module: 'users',
					action: 'unlove',
			...
			Services.invoke(args, unloveSuccess, commandFail, el);
        },
