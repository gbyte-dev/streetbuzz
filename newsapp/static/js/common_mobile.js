...
var STXMOBILEMOBILE = function () {
    ...
    function hashManager(hash) {
        if (hash) {
            ...
            switch (hashParams[0]) {
                case 'add-comment':
                    if (STXMOBILE.isElementInViewport($('.comments-editor'))) {
                        $('.comments-editor textarea').focus();
                    } else {
                        $('body').scrollTo($('.comments-editor'), 800, { offset: -(STXMOBILE.getViewportHeight() - $('.comments-editor').height()) });
    ...
    function dropdownMenu() {
        ...
        $('.menu-btn').live('click', function (e) {
			...
			$('.menu-options').css("dispaly","block");
    ...
    function searchBoxInit() {
    	$('#header-search-mob .menu-options a').live('click', function(e) {
    		e.preventDefault();
    		$('#search-lookin').val($(this).data('type'));
    		
    		$('#header-search-mob .menu-btn').text($(this).text());
      ...
      $('.search-field-mob').focus(function () {
    ...
    return {
    	...
    	*/
        ...
        isElementInViewport: function (el) {
            ...
            return (scrolltop > ($(el).offset().top + $(el).height()) || scrolltop + STXMOBILE.getViewportHeight() < $(el).offset().top + $(el).height()) ? false : true;
        },
        ...
        colorboxInit: function (el) {
            ...
            if (el) {
                $('.attachments', el).each(function () {
                    tmpRel = STXMOBILE.generateToken();
...
$(document).ready(function () {
    STXMOBILEMOBILE.init();
																																	   $(".ctypes").click(function(){
		var dtype   =$(this).attr("data-type");
		$("#hserch-mob").val(dtype);
	});
