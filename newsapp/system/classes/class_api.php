<?php
 	class API
	{
	  ...
	  {	      global $db2;
	    ...
	    function signUpUser($fullname, $email, $phone, $username, $password, $dob, $gender, $location, $location_id=0)
	    {
	   	    global $db2, $C;
	...
	function editUser($userid,$fullname,$email,$phone,$username,$password,$dob,$gender,$location)
	  ...
	  public function loadPosts($user_id,$pagenumber,$pagerecordcount)
	  {
			...
			$query = 'SELECT 
                        ...
                        FROM post_userbox b 
                         ...
                         WHERE
                        b.user_id='.$user_id.' AND p.post_level is null AND p.id is not null and (INSTR (pa.data, "<div") < 1 or INSTR (pa.data, "<div") is null)
	  ...
	  public function expiretoken($userid,$token)
	  {
	    ...
	    public function sbappversion($userid,$token)
	  {
	        ...
	        $res = $db2->query('SELECT * FROM settings WHERE word="SB_APP_VERSION"  LIMIT 1');
	        $forceres = $db2->query('SELECT * FROM settings WHERE word="SB_APP_FORCE_UPDATE"  LIMIT 1');
	        $returr_arr = [];
	        $returr_arr["sb_app_version"] = "";
	         $returr_arr["sb_force_update_app"] = "";

		    if($db2->num_rows($res) > 0)
		    {
			    $obj = $db2->fetch_object($res);
			    $returr_arr["sb_app_version"] =  $obj->value;
			    
			  
		    }
		    if($db2->num_rows($forceres) > 0)
		    {
			    $forceobj = $db2->fetch_object($forceres);
			    $returr_arr["sb_force_update_app"] =  $forceobj->value;
