<?php
	...
	class newpost
	{
		...
		public function save()
		{
			...
			if( ! $this->id )
			{
				if( $is_private ) {
					...
					if($str =="false"){
				    $db2->query('INSERT INTO posts SET api_id="'.$db_api_id.'", user_id="'.$db_user_id.'", group_id="'.$db_group_id.'", mentioned="'.$db_mentioned.'", posttags="'.$db_posttags.'", attached="'.$db_attached.'", date="'.$db_date.'", date_lastcomment="'.$db_date.'", ip_addr="'.$db_ip_addr.'",location="'.$db_geolocation.'" ');
                    }else{
					$db2->query('INSERT INTO posts SET api_id="'.$db_api_id.'", user_id="'.$db_user_id.'", group_id="'.$db_group_id.'", message="'.$db_message.'", mentioned="'.$db_mentioned.'", posttags="'.$db_posttags.'", attached="'.$db_attached.'", date="'.$db_date.'", date_lastcomment="'.$db_date.'", ip_addr="'.$db_ip_addr.'",location="'.$db_geolocation.'" ');
		...
		public function comment_post($postid){
			...
			$this->attachments_copy_files();
				$db2->query('UPDATE  posts_pr SET date_lastcomment="'.$db_date.'" WHERE id="'.$postid.'" ');

			$db2->query('INSERT INTO posts_pr_comments SET api_id="'.$db_api_id.'", post_id="'.$postid.'", user_id="'.$db_user_id.'", message="'.$db_message.'", mentioned="'.$db_mentioned.'", posttags="'.$db_posttags.'", date="'.$db_date.'", ip_addr="'.$db_ip_addr.'"   ');
			if( ! $id = $db2->insert_id() ) {
				return FALSE;
			}
			foreach($this->attached as $k=>$v) {
