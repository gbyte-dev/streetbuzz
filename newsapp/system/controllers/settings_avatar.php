<?php
	...
	$rows = array(
			$table->textField( $this->lang('st_avatar_current_picture'), '<img src="'. $C->STORAGE_URL .'avatars/thumbs1/'. $u->info->avatar .'" alt="" border="0" /><br /><br />' ),
			$table->fileField( $this->lang('st_avatar_change_picture'), 'profile_avatar', '' ),
			$table->textField( '', '<span style="color:#CB4335; font-size:11px;">JPEG, GIF or PNG; 200x200px or larger.</span><br /><br />' ),
			$table->textField( 'Remove Current', '<a href="'.$C->SITE_URL.'settings/avatar/del:current">'.$this->lang('st_avatar_upload_or_delete').' <span class="glyphicon glyphicon-trash"></span></a>' ),
			$table->textField( '', '<br />' ),
