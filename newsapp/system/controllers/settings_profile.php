<?php
	...
	$comment		= $this->user->info->comment;
	...
	if( isset($_POST['sbm']) ) {
		...
		if( !$error ){
			...
			$bdate_y	= isset($_POST['profile_birth_year'])? intval($_POST['profile_birth_year']) : '';
		 	$comment	= isset($_POST['profile_comment'])? ($_POST['profile_comment']) : '';
			...
			$db2->query('UPDATE users SET fullname="'.$db2->e($name).'", about_me="'.$db2->e($aboutme).'", tags="'.$db2->e($tags).'", gender="'.$db2->e($gender).'", birthdate="'.$db2->e($birthdate).'", location="'.$db2->e($location).'" , comment="'.$db2->e($comment).'" WHERE id="'.$this->user->id.'" LIMIT 1');
	...
	$rows = array(
			...
			$table->textArea( $this->lang('st_profile_comment'), 'profile_comment', $comment ),
