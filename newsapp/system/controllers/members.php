<?php
	...
	//TEMPLATE CODE START
	$tpl = new template( array('page_title' => $C->SITE_TITLE . ' - Members', 'header_page_layout'=>'sc') );
	
	$menu = array( 	array('url' => 'members/tab:all', 			'css_class' => (($tab === 'all')? 'active' : ''), 			'title' => $this->lang('members_tabs_all') )
	);
	if( $this->user->is_logged ){
		$menu[] = array('url' => 'members/tab:ifollow', 		'css_class' => (($tab === 'ifollow')? 'active' : ''), 		'title' => $this->lang('members_tabs_ifollow2') );
