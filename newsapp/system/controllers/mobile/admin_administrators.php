<?php
	...
	if( isset($_POST['admin']) ) {
		...
		if(!$csrf->check_if_valid($_POST['token'])){
			$error = TRUE;
			$errmsg = $this->lang('admin_invalid_session');
		}
		...
		if( !$error ){
			...
			if( $a ) { 
				...
				$error = TRUE;
				$errmsg = $this->lang('admadm_jserr_user1', array('#USERNAME#' => $_POST['admin']));
			}
	...
	$rows = array(
			...
			$table->hiddenField('token', $csrf->get_value()),
