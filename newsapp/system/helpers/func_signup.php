<?php
	...
	{
		global $page, $db2, $user;
		
		if( isset($user->sess['email_confirmed']) ){
			return $user->sess['email_confirmed'];
		}
		...
		$user->sess['email_confirmed'] = $email;
...
// 		}
		...
		if (isset($_REQUEST['oauth_verifier'])) {
			...
			$profile = $twitter->get_profile($_REQUEST['oauth_verifier']);
			$exists = $user->get_user_by_twitter_uid($profile->id);
		
			if (isset($exists->id)) {
		
				$user->login($exists->username, $exists->password);
		
				$page->redirect('dashboard');
			} else {
		
				if ($user->is_logged && isset($user->info)) {
					$this->redirect('dashboard');
				}
		
				if( isset($profile->id, $profile->name, $profile->screen_name) ){		
					$_POST['fullname'] 	= trim($profile->name);
					$_POST['username'] 	= strtolower( trim($profile->screen_name) );
					$_POST['tw_user_id'] = (int) trim($profile->id);
