<?php
	...
	if(!empty($page->params->action) && $page->params->action=='list_events' ){
		...
		if(!empty($page->params->group)){
			...
			WHERE  pu.user_id="'.(int)$user->id.'" AND  e.status=1 AND  ((pu.event_status=1 AND pu.status=1) OR (pu.event_status IS NULL  AND pu.status=1) OR (pu.event_status=5 and pu.status=1) ) AND 
