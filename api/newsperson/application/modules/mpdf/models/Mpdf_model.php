...
class Mpdf_model extends MY_Model {
    ...
    public function getAdsRecords($adid,$fromdate,$todate) {
      /* $this->db->select("p.id,pv.view_date,SUM(pv.cnt) as cnt");
        ...
        $query2 = $this->db->query('select p.id,pv.view_date,SUM(pv.cnt) as cnt,SUM(aci.cnt) as clickcnt from ads_info as ad inner join ads_tags as at on at.ad_id= ad.id 
...
where ad.id="'.$adid.'" AND pv.view_date >= "'.$fromdate.'" AND pv.view_date <= "'.$todate.'"  group by pv.view_date  order by pv.view_date desc'
     ...
     public function getUserdetails($adid) {
         $query2 = $this->db->query('select ad.customer_name,ad.contact_number,ad.sales_person,ad.start_date from ads_info as ad 
where ad.id="'.$adid.'" '
        );
        $results = $query2->result_array();  
        return $results;
